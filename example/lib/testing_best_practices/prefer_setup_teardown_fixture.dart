...
// Test fixture for: prefer_setup_teardown
// Source: lib\src\rules\testing\testing_best_practices_rules.dart

import 'package:saropa_lints_example/flutter_mocks.dart';

enum _NameCal { swedish, finnish }

String? _namesForDate({
  required _NameCal calendar,
  required int month,
  required int day,
}) {
  if (month == 2 && day == 30) {
...
// GOOD: Same SUT call shape with different const arrange values — must NOT
// trigger (signatures include normalized simple-local preface).
void _goodPreferSetupTeardownParameterized() {
  group('namesForDate', () {
    test('valid Swedish Jan 1', () {
      const _NameCal calendar = _NameCal.swedish;
      const int month = 1;
      const int day = 1;
      final String? result = _namesForDate(
        calendar: calendar,
        month: month,
        day: day,
      );
    ...
    test('valid Finnish Jan 1', () {
      const _NameCal calendar = _NameCal.finnish;
      const int month = 1;
      const int day = 1;
      final String? result = _namesForDate(
        calendar: calendar,
...
// BAD: identical non-literal setup in 3+ tests — should trigger.
// expect_lint: prefer_setup_teardown
void _badPreferSetupTeardownDuplicated() {
  group('svc', () {
    test('dup a', () {
      final repo = _DupRepo();
      final sut = _DupSut(repo);
      expect(sut.id, 1);
    });
    test('dup b', () {
      final repo = _DupRepo();
