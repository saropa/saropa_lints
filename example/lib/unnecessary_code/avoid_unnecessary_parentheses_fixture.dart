...
// Test fixture for: avoid_unnecessary_parentheses
...
void _printAny(dynamic v) {}

// BAD: Should trigger avoid_unnecessary_parentheses
void _bad() {
  // Parens around a literal do nothing in a variable initializer.
  final x = (5); // LINT

  // No surrounding operator requires this grouping.
  final y = (a + b); // LINT

  // Parens around a standalone return expression.
  int f() {
    return (intValue); // LINT
  }

  // Double-wrapped parens: the outer pair is pure noise.
  final z = ((a + b)); // LINT

  // Redundant parens around a bare positional call argument.
  _printAny((a)); // LINT

  // Redundant parens around a list element.
  final list = [(a), b]; // LINT

  // Redundant parens around an if condition (grammar already parenthesizes
  // the condition; this is a double-wrap).
  if ((a == b)) {} // LINT

  // Redundant parens around a simple identifier used as a cascade target.
  final sb = (StringBuffer())..write('x'); // LINT
}

// GOOD: Should NOT trigger avoid_unnecessary_parentheses
void _good() {
  // Required: changes evaluation order vs a + b * c.
  final x = (a + b) * c;

  // Required: negates the whole sum, not just a.
  final y = -(a + b);

  // Common, intentional style allowance around is/&&.
  final ok = (a is String) && b;

  // Required: 5.toString() fails to lex (the `.` is swallowed as the start
  // of a double literal), so the parens are load-bearing here.
  final s = (5).toString();

  // Required to disambiguate the cascade receiver from `a + b..toString()`.
  final combined = (a + b)..toString();

  // Required to invoke the function expression immediately.
  final result = (() => 1)();

  // Never touched: parens inside string interpolation.
  final message = '${(a + b)}';
}
