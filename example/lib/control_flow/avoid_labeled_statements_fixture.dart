...
// Test fixture for: avoid_labeled_statements
...
// BAD: labeled statement used to break out of nested loops — labels are
// a control-flow escape hatch that is easy to misread as a variable or
// case label; prefer extracting a helper function that `return`s instead.
void search() {
  outer: // LINT — labeled statement
...
// GOOD: no label needed — returning from the enclosing function achieves
// the same "stop searching" behavior without introducing a label.
void searchGood() {
  for (final row in grid) {
    for (final value in row) {
      if (value == target) return; // OK — no label
    }
  }
}
