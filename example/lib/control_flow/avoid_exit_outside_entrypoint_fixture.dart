...
// Test fixture for: avoid_exit_outside_entrypoint
// Source: lib\src\rules\control_flow_rules.dart

import 'dart:io';

// BAD: exit() called outside main() — abrupt process termination hides
// cleanup, makes the function untestable, and is surprising to callers
// that did not expect a full process exit from a leaf function.
void validateConfig() {
  exit(1); // LINT — exit outside main
}

// BAD: same problem in a nested helper function.
void helper() {
  exit(0); // LINT — exit outside main
}

// GOOD: exit() is only allowed directly in main(), the documented
// process entrypoint, where terminating the process is the expected
// and intentional outcome.
void main() {
  exit(1); // OK — inside main
}
