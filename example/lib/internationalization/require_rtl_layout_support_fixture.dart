...
// Test fixture for: require_rtl_layout_support
// Source: lib/src/rules/ui/internationalization_rules.dart

import 'package:saropa_lints_example/flutter_mocks.dart';

// ============================================================================
// BAD: hardcoded directional values outside any enum-mapper context
// ============================================================================

// expect_lint: require_rtl_layout_support
final _badAlignment = Alignment.topLeft;

// expect_lint: require_rtl_layout_support
final _badAlignmentRight = Alignment.bottomRight;

// expect_lint: require_rtl_layout_support
final _badTextAlign = TextAlign.left;

// ============================================================================
// GOOD: directional equivalents
// ============================================================================

final _goodAlignment = AlignmentDirectional.topStart;
final _goodEdgeInsets = EdgeInsetsDirectional.only(start: 1);
final _goodTextAlign = TextAlign.start;

// ============================================================================
// GOOD (false-positive regression): physical-corner enum-mapper pattern.
// The enum's member names commit the public API to physical-direction
// semantics; mapping `EnumValue.name => Alignment.name` is identity-by-name
// and would be wrong to flip under RTL. See bug:
//   bugs/require_rtl_layout_support_false_positive_physical_corner_enum_mapper.md
// ============================================================================

enum AlignmentOption { topLeft, topRight, bottomLeft, bottomRight }

class _PhysicalCornerMapperArrowBody {
  // Arrow-bodied switch expression — the natural shape from the bug report.
  Alignment toAlignment(AlignmentOption value) => switch (value) {
    AlignmentOption.topLeft => Alignment.topLeft,
    AlignmentOption.topRight => Alignment.topRight,
    AlignmentOption.bottomLeft => Alignment.bottomLeft,
    AlignmentOption.bottomRight => Alignment.bottomRight,
  };
}

class _PhysicalCornerMapperBlockBody {
  // Block body with `return switch` — same semantic, different syntax.
  Alignment toAlignment(AlignmentOption value) {
    return switch (value) {
      AlignmentOption.topLeft => Alignment.topLeft,
      AlignmentOption.topRight => Alignment.topRight,
      AlignmentOption.bottomLeft => Alignment.bottomLeft,
...
enum TextAlignOption { left, right }

class _PhysicalTextAlignMapper {
  // TextAlign symmetry — same identity-mapper pattern.
  TextAlign toAlign(TextAlignOption value) => switch (value) {
...
// ============================================================================
// BAD (regression): non-identity mapping inside a switch arm must still lint —
// the fix must NOT silence cases where the enum name does not match the
// Alignment member name (such mappings can be RTL-converted safely).
// ============================================================================

enum DirectionOption { start, end }

class _NonIdentityMapper {
  Alignment toAlignment(DirectionOption value) => switch (value) {
    // expect_lint: require_rtl_layout_support
    DirectionOption.start => Alignment.topLeft,
    // expect_lint: require_rtl_layout_support
    DirectionOption.end => Alignment.bottomRight,
  };
}
