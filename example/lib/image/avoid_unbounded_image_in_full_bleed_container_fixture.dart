...
// Test fixture for: avoid_unbounded_image_in_full_bleed_container
// Source: lib\src\rules\media\image_rules.dart

import 'package:saropa_lints_example/flutter_mocks.dart';

// BAD: no size/cache bound, Stack forces expand
// expect_lint: avoid_unbounded_image_in_full_bleed_container
void _bad901() {
  Stack(fit: StackFit.expand, children: [Image.asset('assets/hero.png'), const Text('Overlay')]);
}

// BAD: no size/cache bound, Positioned.fill forces expand
// expect_lint: avoid_unbounded_image_in_full_bleed_container
void _bad902() {
  Stack(children: [Positioned.fill(child: Image.network('https://example.com/bg.jpg'))]);
}

// BAD: no size/cache bound, SizedBox.expand forces expand
// expect_lint: avoid_unbounded_image_in_full_bleed_container
void _bad903() {
  SizedBox.expand(child: Image.memory(bytes));
}

// GOOD: width/height are set, so decode is already bounded
void _good901() {
  Stack(
    fit: StackFit.expand,
    children: [
      Image.asset('assets/hero.png', width: 400, height: 300),
      const Text('Overlay'),
    ],
  );
}

// GOOD: cacheWidth/cacheHeight are set, so decode is already bounded
void _good902() {
  Positioned.fill(
    child: Image.network(
...
// GOOD: no full-bleed ancestor at all (plain layout, own intrinsic size)
void _good903() {
  Column(children: [Image.asset('assets/thumb.png'), const Text('Caption')]);
}

// GOOD: a nearer SizedBox constrains the image before the full-bleed
// Stack is reached -- the Stack's expand fit is irrelevant to this Image.
void _good904() {
  Stack(
    fit: StackFit.expand,
    children: [
      SizedBox(width: 40, height: 40, child: Image.asset('assets/icon.png')),
    ],
  );
}
