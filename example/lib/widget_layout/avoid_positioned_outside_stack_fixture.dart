...
// Test fixture for: avoid_positioned_outside_stack
// Source: lib\src\rules\widget\widget_layout_constraints_rules.dart
...
// ---------------------------------------------------------------------------
// BAD: Positioned directly inside Column — true positive
// ---------------------------------------------------------------------------

// expect_lint: avoid_positioned_outside_stack
void _badPositionedInColumn() {
  Column(
    children: [
      Positioned(top: 10, child: Text('crash')), // LINT
    ],
  );
}

// ---------------------------------------------------------------------------
// GOOD: Positioned directly inside Stack — no lint
// ---------------------------------------------------------------------------

void _goodPositionedInStack() {
  Stack(
    children: [
      Positioned(top: 10, child: Text('ok')),
    ],
  );
}

// ---------------------------------------------------------------------------
// GOOD: Positioned inside IndexedStack — no lint
// ---------------------------------------------------------------------------

void _goodPositionedInIndexedStack() {
  IndexedStack(
    children: [
      Positioned(top: 10, child: Text('ok')),
    ],
...
// ---------------------------------------------------------------------------
// GOOD: Positioned inside a list passed to a custom widget parameter.
// The custom widget may spread the list into its own Stack internally.
// The rule cannot see inside the widget, so it must be conservative.
// Covers: FP fix from 2026-05-13 (backgroundLayers pattern).
// ---------------------------------------------------------------------------

/// Custom widget that owns a Stack and spreads caller-supplied layers.
/// Defined here (not in flutter_mocks.dart) so _isCustomFlutterWidget
/// classifies it as a user widget and the indeterminate path fires.
class FocusCard extends StatelessWidget {
  const FocusCard({required this.backgroundLayers, super.key});

  /// Positioned entries land as direct Stack children at runtime.
...
void _goodPositionedInListPassedToCustomWidget() {
  // No lint — FocusCard is a user widget, so the rule returns indeterminate.
  FocusCard(
    backgroundLayers: <Widget>[
      Positioned(right: -16, bottom: -16, child: Text('star')),
    ],
  );
}

// ---------------------------------------------------------------------------
// GOOD: Positioned built via a helper method that returns it.
// The rule returns indeterminate at non-build method boundaries.
// ---------------------------------------------------------------------------

Widget _buildOverlay() {
  return Positioned(top: 0, child: Text('overlay'));
}
...
void _goodPositionedInMapCallback() {
  Stack(
    children: [1, 2, 3].map((i) {
      return Positioned(top: i.toDouble(), child: Text('$i'));
    }).toList(),
  );
}
