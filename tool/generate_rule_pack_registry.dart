...
void main() {
  ...
  _writeRuleTierDefinitionsTs(root: root);
...
/// Suffix that marks a stylistic category as a mutually-exclusive group.
/// Categories carrying this in their name render as pick-one radios in the
/// config dashboard; all others render as independent toggles.
const String _conflictMarker = '(conflicting - choose one)';

/// Convert a human category name into a stable snake_case id for the extension.
/// Strips the conflict marker first so the id reflects the concept, not the
/// pick-one annotation.
String _stylisticPackId(String categoryName) {
  final cleaned = categoryName.replaceAll(_conflictMarker, '').trim();
  final lower = cleaned.toLowerCase();
  final buf = StringBuffer();
  var lastWasUnderscore = false;
  for (final ch in lower.split('')) {
    final isWord = RegExp(r'[a-z0-9]').hasMatch(ch);
    if (isWord) {
      buf.write(ch);
...
/// Human label for a stylistic pack: the category name minus the conflict marker.
String _stylisticPackLabel(String categoryName) =>
    categoryName.replaceAll(_conflictMarker, '').trim();

/// Emit `extension/src/rulePacks/stylisticPackDefinitions.ts` from
/// [stylistic.stylisticRuleCategories], the single source of truth shared with
/// the Dart init wizard. Conflicting categories become `selectionMode: 'pickOne'`
/// (radio group); the rest become `selectionMode: 'multi'` (independent toggles).
///
/// Stylistic rules are off in every tier, so the dashboard treats each rule as
/// an explicit per-rule opt-in written to the RULE OVERRIDES section, not as a
/// tier or dependency-gated pack. That is why this registry carries no pubspec
/// markers or SDK gates.
void _writeStylisticPackDefinitionsTs({required Directory root}) {
  final categories = stylistic.stylisticRuleCategories;
  final tsFile = File(
    '${root.path}/extension/src/rulePacks/stylisticPackDefinitions.ts',
  );
  ...
  sb.writeln(' * SDK gates apply — these are pure style choices.');
  sb.writeln(' *');
  sb.writeln(
  ...
  sb.writeln('export interface StylisticPackDefinition {');
  sb.writeln('  /** Stable snake_case id derived from the category name. */');
  sb.writeln('  readonly id: string;');
  sb.writeln('  /** Display name (category name minus the conflict marker). */');
  sb.writeln('  readonly label: string;');
  sb.writeln(
    "  /** 'pickOne' renders a radio group (mutually exclusive); 'multi' renders toggles. */",
  );
  sb.writeln("  readonly selectionMode: 'multi' | 'pickOne';");
  sb.writeln('  /** Lint rule codes in this group (off by default). */');
  ...
  for (final entry in categories.entries) {
    final id = _stylisticPackId(entry.key);
    final label = _stylisticPackLabel(entry.key);
    final mode = entry.key.contains(_conflictMarker) ? 'pickOne' : 'multi';
    sb.writeln('  {');
    sb.writeln("    id: '$id',");
    sb.writeln("    label: '${label.replaceAll("'", r"\'")}',");
    sb.writeln("    selectionMode: '$mode',");
...
/// The tier that first introduces [rule], honoring the cumulative tier model
/// (essential ⊂ recommended ⊂ professional ⊂ comprehensive ⊂ pedantic). Each
/// `*OnlyRules` set holds the rules ADDED at that level, so the first matching
/// set in ascending order is the rule's introducing tier. Stylistic rules sit
/// outside the cumulative chain (off in every tier). `unknown` is unreachable
/// for any rule in getAllDefinedRules(), but kept as a defensive fallback so a
/// future rule that escapes every set still groups somewhere instead of throwing.
String _introducingTier(String rule) {
  if (tiers.essentialRules.contains(rule)) return 'essential';
  if (tiers.recommendedOnlyRules.contains(rule)) return 'recommended';
  if (tiers.professionalOnlyRules.contains(rule)) return 'professional';
  if (tiers.comprehensiveOnlyRules.contains(rule)) return 'comprehensive';
  if (tiers.pedanticOnlyRules.contains(rule)) return 'pedantic';
  if (tiers.stylisticRules.contains(rule)) return 'stylistic';
  return 'unknown';
}

/// Emit `extension/src/rulePacks/ruleTierDefinitions.ts` — a flat
/// `ruleCode → introducing tier` map so the Findings dashboard can group
/// findings by tier client-side (no plugin re-scan, mirroring how
/// RULE_PACK_DEFINITIONS lets it group by pack). tiers.dart is the single
/// source of truth; this is its TS projection, regenerated alongside the pack
/// registry whenever rules move between tiers.
void _writeRuleTierDefinitionsTs({required Directory root}) {
  final allRules = tiers.getAllDefinedRules().toList()..sort();
  final tsFile = File(
    '${root.path}/extension/src/rulePacks/ruleTierDefinitions.ts',
  );
  final sb = StringBuffer();
  sb.writeln('/**');
  sb.writeln(
  ...
  sb.writeln(' */');
  sb.writeln();
  sb.writeln('export const RULE_TIER_BY_CODE: Readonly<Record<string, string>> = {');
  for (final rule in allRules) {
    sb.writeln("  '$rule': '${_introducingTier(rule)}',");
  }
  sb.writeln('};');
  tsFile.writeAsStringSync(sb.toString());
  print('Wrote ${tsFile.path} (${allRules.length} rules)');
}
