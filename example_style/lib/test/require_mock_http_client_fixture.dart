...
// Test fixture for: require_mock_http_client
...
class MockClient {
  dynamic get(dynamic url) => null;
  dynamic post(dynamic url) => null;
}
...
// BAD: Real HTTP call with http.get in test
...
// BAD: Real HTTP call with client.post in test
// expect_lint: require_mock_http_client
void _bad2() {
  test('creates user', () async {
    final response = await client.post(Uri.parse('https://api.example.com'));
    expect(response, isNotNull);
  });
}

// GOOD: Using MockClient
void _good1() {
  test('fetches user with mock', () async {
