...
- **Publish script: delta-first testing** — the test step now automatically detects changed files via `git diff`, maps them to corresponding test files, and runs only those first (seconds instead of minutes). If delta passes, the full suite is deferred to CI. Infrastructure changes (tiers, registration, pubspec) bypass delta and run the full suite.
- **Seven untagged integration tests now marked `@Tags(['slow'])`** — cross-file CLI, vibrancy CLI, and deadweight integration tests were running in the "fast" publish pass and timing out at 2 minutes each, adding 8+ minutes of wasted time. They are now excluded locally and run only in CI.
...
- **Publish script: no more forced retry on real test failures** — a failing test pass used to always re-run once automatically before asking the user anything, silently doubling the wait on a run that was never going to pass. The automatic retry now only fires when the failure is diagnosed as transient (VM crash or file lock); a genuine test failure goes straight to the Continue/Retry/Abort prompt so the user decides whether to spend more time re-running.
