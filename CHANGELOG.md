...
- **`check_l10n_keys.py`** — new CI script cross-references every `l10n()` call in `extension/src/` against `en.json`, with `--check-params` to validate interpolation tokens match between call sites and catalog values. Proper comment stripping, nested-paren-aware param extraction, and dynamic-key-prefix filtering. No action required.
