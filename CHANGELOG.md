...
- `use_setstate_synchronously` no longer fires on a `setState` that lexically precedes the first `await` when both calls live inside a single compound statement (try/if/for/switch); the walker now tracks await position and `if (!mounted) return;` guard scope in source order across nested blocks, so codebases that wrap method bodies in mandatory `try { … } on Object catch` no longer see the structural false positive. Genuine post-await unguarded cases still warn. No action required.
