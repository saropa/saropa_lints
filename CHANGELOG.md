...
- **"Saropa Dashboards" is now a Home hub with a live KPI band.** The dashboard opened by the status bar and the sidebar's "Dashboards" row now leads with a 6-tile strip (health score, open issues, diagnostic engines, packages, Code Health grade, project size) plus one card per other dashboard (Findings, Rules & Tiers, Packages, Code Health, Project Map, Full Audit) showing its top signals and an Open button. Every value is a cheap read — nothing on this page triggers a full-project scan, so it always opens instantly. The older "Saropa Lints" consolidated dashboard (grade gauge + rule list) is removed; its role is covered by the new KPI band.
- **19 new tier-1 quick-win lint rules.** `avoid_disposing_late_fields` flags `.dispose()` on conditionally-initialized `late` fields (Recommended). `avoid_dynamic_calls` catches method/property/index access through resolved `dynamic` (Recommended). `avoid_equals_and_hash_code_on_mutable_classes` prevents `==`/`hashCode` overrides on classes with non-final fields (Essential). `avoid_futureor_return_type` flags `FutureOr<T>` as a declared return type (Recommended). `avoid_implementing_value_types` catches classes that `implements` a known value-equality type (Comprehensive). `avoid_mounted_check_in_finally` flags `mounted` guards inside `finally` blocks (Recommended). `document_enum` requires doc comments on public enums and enum values (Stylistic opt-in). `duplicate_value` detects repeated sub-expressions in boolean chains (Recommended). `getters_in_member_list` flags getters declared after behavior members (Stylistic opt-in). `initializers_ordering` enforces field-declaration order in constructor initializer lists (Stylistic opt-in). `is_future` catches runtime `x is Future` type checks (Recommended). `mutable_tearoff` flags method tear-offs from non-final fields (Professional). `named_parameters_ordering` enforces declaration-order named arguments at call sites (Stylistic opt-in). `never_discard_build_context` catches unused `BuildContext` params in builder callbacks (Recommended). `new_instance_cascade` suggests cascades for consecutive statements on a freshly-created instance (Stylistic opt-in). `no_direct_iterable_access` flags `list[i]` index access without a bounds guard (Professional). `prefer_typed_exceptions` catches `throw` of raw String/non-Exception values (Comprehensive). `specify_unknown_enum_value` requires `unknownEnumValue` on `@JsonSerializable` enum fields (Comprehensive). `use_compare_without_case` flags `toLowerCase() ==` patterns that should use `compareTo` (Stylistic opt-in). No action required.
