...
A new project size scan lays the groundwork for an upcoming Project Map dashboard. Run it to see which files and folders are the biggest by lines and on-disk size, so oversized files are easy to spot. The Code Health Dashboard now opens right away and fills in live as it scans — a progress bar, the file being processed, and Pause/Resume/Restart/Cancel controls — instead of a notification that looked stuck, and it no longer crashes when a source file can't be fully parsed. [log](https://github.com/saropa/saropa_lints/blob/main/CHANGELOG.md)
