...
- **Config as control surface (C6):** Changing tier or running "Initialize / Update Config" now auto-runs analysis (when `runAnalysisAfterConfigChange` is on) so the user sees the score delta immediately.
