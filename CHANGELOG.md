...
New rule `prefer_animation_controller_forward_from_zero` flags `AnimationController.forward()` (no args) inside press/tap gesture callbacks when the controller is wired to auto-reverse on completion — the canonical press-and-bounce pattern. Without `from: 0.0`, a rapid re-press while the controller is mid-reverse resumes from the in-flight value and the animation plays only part of its duration, making rapid taps feel sticky and inconsistent. New rule `prefer_single_ticker_provider_state_mixin` flags `State` classes that mix in `TickerProviderStateMixin` while declaring only one `AnimationController`, recommending the cheaper `SingleTickerProviderStateMixin` variant (one-token quick fix). The VS Code extension's `Run Analysis` command stops dumping `dart analyze`'s progress-bar stderr into the warning popup — it now reports the real issue count from `violations.json` with `View Violations` / `Show Output` buttons. The rule formerly known as `depend_on_referenced_packages` is now `saropa_depend_on_referenced_packages` to stop it from double-reporting alongside the Dart SDK lint of the same name — and, much more importantly, the underlying pubspec dependency parser that made that rule fire on literally every `package:` import (22,695 false positives on `saropa-contacts`, 96.8% of the entire backlog) has been repaired: a missing `multiLine: true` flag on the parser's regex was returning an empty dependency set on every project, silently breaking every rule in the plugin that gates itself on "skip when the package IS declared".
