...
- Project Vibrancy now has primary extension UI surfaces: a dedicated sidebar webview (filters, quick unused/uncovered slices, persisted filter state, and `--since` git-ref scoped scans) plus a full report webview command with clickable function hotspots, so teams can use project-level code-health scoring directly in the IDE instead of CLI-only output. No action required beyond updating the extension and opening **Project Vibrancy** from the Saropa sidebar.
...
- Cross-file CLI now includes a first-pass `dead-imports` mode for likely dead relative imports, with extension command support, so teams can spot stale local imports during architecture cleanup without custom scripts; advanced cases (re-exports and deferred imports) remain future work in this pass. No action required unless you want to run the new command and review candidates.
...
- Cross-file `watch` mode now prints per-rerun delta summaries (new vs resolved findings, or import-stat deltas), so ongoing edits are easier to track than re-reading full output each time. No action required.
