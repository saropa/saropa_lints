...
- **Bulk-copy methods for analysis** — every Worst Functions row now carries a selection checkbox (with a master checkbox that toggles every currently-visible row), and a *Copy selected (N)* toolbar button copies the picks to the clipboard as one line per row in the format `file:line  function-name  (score N, flags)`. Designed for pasting into a chat / issue / AI prompt for triage. No action required.
