...
Minor release adding a new essential-tier rule that catches an unguarded `dart:developer` `debugger()` call before it can hang a test run. It also fixes a set of problems in the GitHub Actions integration found in a full review: monorepo projects, baselines, `since`, the release tag, and the extension's CI switch and publish step.
