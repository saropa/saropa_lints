...
- Turning Lint integration back on re-ran `pub get` even when nothing had changed, which takes around two minutes on a large Flutter project and reads as a frozen "Running pub get…" notification, so the enable gets canceled and the project appears impossible to re-enable. Enable now reuses the existing resolution when `pubspec.yaml` is untouched and the package is already resolved, and still runs `pub get` whenever the manifest is newer than the resolution. No action required.
