...
- Fixed `avoid_nullable_interpolation` compile error where `_findRegExpPattern` received an `InterpolationExpression` instead of the unwrapped `Expression`, breaking process-level tests that exercise the full binary. No action required.
