...
- **New `dart run saropa_lints:memory_report` command** — summarizes the analysis server's RSS trend from `plugin.log` for post-crash diagnosis. The in-process plugin now writes a memory sample line roughly every 30 seconds; the command reports min/max/latest RSS and percent of the configured cap.
