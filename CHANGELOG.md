...
- Package Vibrancy user-facing text now refers to the **activity-bar list**, **Saropa Package Vibrancy** (editor tab), and **CodeLens** instead of generic “tree view” wording; **Saropa: Open Package Dashboard** is the palette label for `saropaLints.packageVibrancy.showReport` (same command id). No config change.
...
- The **Package Vibrancy** report's row detail (the panel showing **Health Score**, file usage, etc.) now starts properly collapsed in every webview renderer — the inline `style="display:none"` on the detail `<tr>` was being ignored by some hosts (notably the Cursor IDE in certain builds), leaving the panel open while the chevron stayed in its right-pointing collapsed state. Switched to the HTML5 `hidden` attribute and `.hidden` DOM property, which the spec requires every renderer to honor. No action required.
