...
- Phase 2 cross-cutting sweeps: hex-fallback removal in `detail-view-styles.ts` (`var(--vscode-*, #hex)` → `var(--vscode-*)` so dark-only fallbacks no longer break light themes); reduced-motion guard added to chart-script's donut draw-in animation (skips the rAF reset when `prefers-reduced-motion: reduce` is on, end state still applies); pluralization sweep replaces inline `${n === 1 ? '' : 's'}` ternaries across the Findings, Code Health, Comparison, and Telemetry dashboards with the new `pluralize()` helper; logical-property sweep on the chrome stylesheet (`margin-left` / `margin-right` / `border-left` → `margin-inline-start` / `margin-inline-end` / `border-inline-start`) so the layout flips cleanly when a future RTL pass arrives. Loading-verb sweep found no offenders. CSV export hardening N/A — current export paths are JSON / Markdown only; CSV will adopt §22.2 conventions when introduced.
- Phase 3 per-dashboard a11y additions (UX guidelines §15.2 / §15.3): every editor-area dashboard now ships a *Skip to content* link as the first focusable element (visible only on focus), a polite `aria-live` announcer region for filter / sort state changes, and explicit `<header>` / `<main>` / `<aside>` landmark elements that screen readers can navigate via landmark shortcuts. Findings, Code Health, and Known Issues additionally announce visible-row counts on every filter change. Comparison announces the package being added on *Add to Project*. The Catalog renames its main region id from `catalog` to `catalog-main` to match the skip-link target; the corresponding script reference updates with it. Eight dashboards covered: Findings, Code Health, Comparison, Rule Explain, Related Rule Telemetry, Command Catalog, Single-package detail, Known Issues. About panel SKIPPED — the static read-only view has no script context and no filter / sort state to announce.
- Phase 4 ran the token-coverage matrix generator and committed the first snapshot (`extension/src/test/views/snapshots/token-matrix.md`, 66 unique tokens × 13 surfaces). The compliance plan in `plan/UX_GUIDELINES_COMPLIANCE.md` is updated with phase-by-phase checkboxes, per-surface status table, and explicit *deferred* markers for the opt-in polish items (multi-column sort, multi-select + bulk actions, what-is-this disclosure, column resize/reorder, dedicated no-project webview, per-section error boundary) that are documented as available patterns but blocked on real product complaints rather than gateable on a release.
...
> See [CHANGELOG_ARCHIVE.md](https://github.com/saropa/saropa_lints/blob/main/CHANGELOG_ARCHIVE.md) for versions 0.1.0 through 12.5.1.
