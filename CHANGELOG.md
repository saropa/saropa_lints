...
- **False positive reduction (Phase 2 batch 6):** **api_network_rules:** `require_websocket_error_handling`, `avoid_websocket_without_heartbeat`, `prefer_timeout_on_requests`, `require_permission_rationale`, `require_permission_status_check`, `require_notification_permission_android13`, `require_sqflite_migration`, `require_websocket_reconnection` — replaced targetSource/bodySource/classSource `.contains()` with word-boundary regex or exact sets. CI baseline: api_network_rules 51→17.
- **False positive reduction (Phase 2 batch 7):** **api_network_rules:** `require_typed_api_response`, `require_response_caching`, `prefer_api_pagination`, `require_image_picker_result_handling`, `require_sse_subscription_cancel` — replaced bodySource/methodName/source/typeName/disposeBody `.contains()` with regex or helpers. api_network_rules reached 0 violations; baseline entry removed from `anti_pattern_detection_test.dart`.
- **False positive reduction (async_rules):** **async_rules:** `avoid_dialog_context_after_async`, `require_websocket_message_validation`, DateTime UTC storage rule, `require_loading_timeout`, `prefer_broadcast_stream`, mounted/setState visitors, `require_network_status_check`, `require_pending_changes_indicator`, `avoid_stream_sync_events` — replaced bodySource/methodName/.toSource() `.contains()` with word-boundary regex or static pattern lists. async_rules reached 0 violations; baseline entry removed.
- **False positive reduction (navigation_rules):** **navigation_rules:** `avoid_circular_redirects`, `require_deep_link_fallback`, `prefer_typed_route_params`, `require_stepper_validation`, `require_step_count_indicator`, `require_go_router_typed_params`, `require_url_launcher_encoding`, `avoid_navigator_context_issue` — replaced bodySource/methodName/source `.contains()` with word-boundary regex or static pattern lists. navigation_rules reached 0 violations; baseline entry removed.
- **False positive reduction (file_handling_rules):** **file_handling_rules:** PDF, sqflite (whereargs, transaction, error handling, batch, close, reserved word, singleton, column constants), large-file rule, require_file_path_sanitization — replaced targetSource/bodySource/source `.contains()` with word-boundary regex or static pattern lists. file_handling_rules reached 0; baseline entry removed.
- **False positive reduction (security_rules):** **security_rules:** RequireSecureStorageRule, RequireBiometricFallbackRule, AvoidStoringPasswordsRule — pref/shared, auth/bio, password key patterns; RequireTokenRefreshRule, AvoidJwtDecodeClientRule, RequireLogoutCleanupRule — refresh/expiry, jwt/decode/token, logout cleanup patterns; RequireDeepLinkValidationRule, AvoidPathTraversalRule, RequireDataEncryptionRule — queryParameters/pathSegments, path validation, secure storage target. AvoidLoggingSensitiveDataRule — _isSafeMatch uses RegExp. Baseline: security_rules 74→41.
- **False positive reduction (security_rules batch 2):** **security_rules:** Remaining targetSource/methodName/bodySource/source `.contains()` replaced with word-boundary RegExp lists or RegExp.escape: RequireSecureStorageForAuthRule, AvoidRedirectInjectionRule, PreferLocalAuthRule, RequireSecureStorageAuthDataRule, AvoidStoringSensitiveUnencryptedRule, HTTP client rule, RequireCatchLoggingRule, RequireSecureStorageErrorHandlingRule, AvoidSecureStorageLargeDataRule, RequireClipboardPasteValidationRule, OAuth PKCE rule, session timeout rule, AvoidStackTraceInProductionRule, RequireInputValidationRule. security_rules reached 0 violations; baseline entry removed from `anti_pattern_detection_test.dart`.
- **False positive reduction (disposal_rules):** **disposal_rules:** Replaced all `typeName.contains(...)` with word-boundary RegExp: media/Tab/WebSocket/VideoPlayer/StreamSubscription/ReceivePort/Socket types and disposable-type loops. disposal_rules reached 0; baseline entry removed from `anti_pattern_detection_test.dart`.
...
- **Discussion 062 (false positive reduction review):** Completed; moved to `bugs/history/discussion_062_false_positive_reduction_review.md`. Ongoing guidance remains in CONTRIBUTING.md § Avoiding False Positives and `project skill doc` § Reducing False Positives.
