...
Adds lint rules for eight more packages — quick_actions, in_app_review, local_auth, file_picker, device_calendar, home_widget, geocoding, and image_picker. The rules catch the documented crash and contract traps for each: shortcuts that do nothing on a cold-start tap, review prompts fired against store guidelines, discarded biometric-auth results, file-picker cancel/web/custom-type mistakes, calendar permission and timezone bugs, home-widget callbacks tree-shaken in release, geocoding crashes on empty results, and image_picker lost-data, range, and empty-result traps. Each package's rules run only in files that import it, so projects that do not use a package see nothing. No action required.
