...
// Test fixture for: require_auto_route_full_hierarchy
...
// BAD: push() bypasses route hierarchy
// expect_lint: require_auto_route_full_hierarchy
void _badPush() {
  context.router.push(const ProfileRoute());
}

// BAD: push() with non-const route
// expect_lint: require_auto_route_full_hierarchy
void _badPush2() {
  context.router.push(ProductDetailRoute(id: 42));
}

// GOOD: navigate() preserves route hierarchy
void _goodNavigate() {
  context.router.navigate(const ProfileRoute());
}

// GOOD: navigate() with parameter
void _goodNavigate2() {
  context.router.navigate(ProductDetailRoute(id: 42));
}

// FALSE POSITIVE: push with string argument (caught by context_navigation rule)
void _fpStringPush() {
  context.router.push('/profile');
}

// FALSE POSITIVE: push called on non-router target
void _fpNonRouter() {
  final list = [];
  list.push(42);
}
