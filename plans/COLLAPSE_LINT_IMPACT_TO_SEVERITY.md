...
- [~] **SEV-01 (P1)** Audit `severity:` assignments rule-by-rule against MUST-fix / should-fix / info guidance. (Bucket A DONE 2026-06-10; Bucket B open. Full cross-check of all 2159 rules' `severity:` field vs `impact` getter — see [`SEV01_SEVERITY_AUDIT.md`](SEV01_SEVERITY_AUDIT.md). **Bucket A (over-rated, ERROR with explicit impact≤warning) is fully audited:** every such rule was read individually; **46 downgraded ERROR→WARNING** (11 Riverpod/Bloc preferences + should-fix patterns, then a 35-rule sweep across performance/robustness-fallback/deployment-config/accessibility/test-quality rules), each with an inline `// SEV-01 (downgraded from ERROR):` why-comment; **~32 confirmed-correct ERRORs kept** (compile errors, deterministic crashes, data corruption/loss, security/exploit, disposal-leak family). Verified: 2286 affected rule tests pass + `dart analyze --fatal-infos` clean. **Still open — Bucket B only:** 46 WARNING-severity rules whose explicit impact=error suggests under-rating. The UPGRADE direction (WARNING→ERROR) turns every false positive into a build-breaking error for consumers, so per the maintainer it requires per-rule false-positive analysis before any change — see the Bucket B plan in the 2026-06-10 finish report below.)
