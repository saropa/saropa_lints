...
Related (same root mechanism, different surface): `plans/history/2026.04/2026.04.25/avoid_ios_hardcoded_status_bar_false_positive_generic_padding.md` covers the `EdgeInsets.only(top: …)` variant. That doc lives in `plans/history/` rather than `bugs/`, contrary to `bugs/BUG_REPORT_GUIDE.md` policy. This report is the SizedBox-specific instance with a concrete downstream reproducer.
...
([generic padding](../plans/history/2026.04/2026.04.25/avoid_ios_hardcoded_status_bar_false_positive_generic_padding.md)).
