# Require Pagination Error Recovery

**GitHub:** [https://github.com/saropa/saropa_lints/issues/22](https://github.com/saropa/saropa_lints/issues/22)

**Opened:** 2026-01-23T13:50:08Z

**Resolved in:** 6.2.1 — Implemented `require_pagination_error_recovery` (Recommended, INFO). Detects ListView.builder/GridView.builder with loadMore/fetchMore/nextPage and warns when enclosing scope has no retry/onError/catch/hasError/isError/errorBuilder. Issue closed.
...
### Problem  
Failed page loads in paginated lists need a retry option. Without error recovery, users may be unable to access data if a single page fails to load.

### Why This Is Complex  
- **Pattern detection:** Requires analyzing error handling in pagination logic.
- **UI feedback:** Error states and retry options may be implemented in various ways.
- **False positives:** Some apps may handle errors outside the pagination component.

### Desired Outcome  
- Detect pagination without error state handling.
- Warn about missing retry options for failed page loads.
- Suggest best practices for error recovery in paginated lists.

### References  
- See ROADMAP.md section: "require_pagination_error_recovery"

---

## Roadmap task spec (merged from bugs/roadmap/task_require_pagination_error_recovery.md)

# Task: `require_pagination_error_recovery`

## Summary
- **Rule Name**: `require_pagination_error_recovery`
- **Tier**: Recommended
- **Severity**: INFO
- **Status**: Planned
...
Failed page loads need retry option. Detect pagination without error state handling.
...
// TODO: Replace with concrete example for `require_pagination_error_recovery`.
```
...
- Add package/docs links relevant to `require_pagination_error_recovery` (e.g. bloc, riverpod, hive).
