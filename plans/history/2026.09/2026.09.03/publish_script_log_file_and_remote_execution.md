...
The publish script gained five CLI flags (`--log-file`, `--log-append`, `--mode`, `--auto-retry`, `--output-level`) and non-interactive execution support. The logging layer tees all `print_*` output through a centralized `log_write` function that strips ANSI codes, with graceful handling of unwritable log paths and append mode for CI runs that accumulate logs. The non-interactive mode auto-answers prompts with safe defaults and auto-retries failed steps (up to the configured limit) before aborting. Mode definitions are unified in a single `_MODE_TABLE` that drives both the interactive menu and CLI flag parsing — adding a mode requires editing one place. Auto-retry is surfaced in both CLI (`--auto-retry <n>`) and the interactive startup prompt (only for pipeline modes, not lightweight audit/analyze modes), per the project's "no buried features" rule. `--output-level` controls console verbosity (silent/warnings/normal/verbose) independently of the log file, which always captures at the configured level. CLI argument parsing uses a `_ParsedArgs` dataclass for clean extensibility. Sub-module `input()` calls remain as incremental migration targets.
