...
- **Tier recommendation:** Pedantic, per the proposal's own justification (pure convention, no correctness impact, opt-in only). Registered in `lib/src/tiers.dart` under `pedanticOnlyRules`, in `_allRuleFactories` (`lib/saropa_lints.dart`), and exported via `all_rules.dart` / `rule_category_map.dart` — the rule is fully live and reachable via the scan CLI and analyzer plugin. (Corrected 2026-09-04 — an earlier draft of this note claimed the opposite; see the 2026-09-04 Finish Report below for why that was wrong.)
- **Detection:** walks `ClassDeclaration` / `MixinDeclaration` / `ExtensionDeclaration` / `EnumDeclaration` / `ExtensionTypeDeclaration` member lists (via the `.bodyMembers` analyzer-12-compat shim in `lib/src/analyzer_compat.dart`, not raw `.members`) in source order, tracking two booleans: `hasEarlierPropertyMember` (a field/getter/setter has been seen) and `sawBehaviorMember` (a regular, non-getter/setter method has been seen). A plain (non-`@override`) getter is flagged only when both are true at the point it's encountered. Enum and extension-type coverage was added 2026-09-04 (see Finish Report) — the original implementation covered only class/mixin/extension.
- **Deviation from the literal proposal text:** constructors are explicitly excluded from `sawBehaviorMember`. The proposal's own BAD/GOOD examples require this — the GOOD example places the constructor *after* the getter, and the BAD example's flagged getter trails a *method*, not the constructor that precedes it. Treating constructors as behavior members caused a false positive on the proposal's own GOOD-shaped test case (constructor-first, then field, then getter, then method — a common Dart idiom); fixed by leaving constructors uncounted by either tracking flag. Policy as later clarified: a constructor is construction/init plumbing, not "what does this object do" behavior, so its position is orthogonal to getter grouping.
- **Edge case 2 (`@override` getters):** resolved as "exempt", per the proposal's own suggestion — an `@override` getter is never flagged regardless of its position. The check is syntactic (`@override` by name), not a resolved-element check, since this rule never resolves types.
...
- **Files:**
  ...
  - `test/rules/code_quality/getters_in_member_list_test.dart` (11 oracle-backed unit tests, all passing — expanded 2026-09-04 to cover setter-as-earlier-property, operator-as-behavior, mixin/extension/enum bodies, and multi-getter independent flagging)
  - `example/lib/code_quality/getters_in_member_list_fixture.dart` (fixture; verified firing at the expected lines via the oracle test harness, including an added enum case)
- **Known limitation:** `static` members are tracked identically to instance members — a body that intentionally separates a `static` block from the instance section can still get flagged. Not fixed; would require a second static/instance tracking axis for a case the source proposal doesn't call out.
