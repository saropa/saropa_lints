...
### Registration (corrected 2026-09-04)

The rule is registered and shipping: `MutableTearoffRule.new` is wired into `_allRuleFactories` in `lib/saropa_lints.dart`, `'mutable_tearoff'` is in `professionalOnlyRules` in `lib/src/tiers.dart`, the export is in `lib/src/rules/all_rules.dart`, and `CHANGELOG.md` documents it under the "19 new tier-1 quick-win lint rules" entry. It is live in the Professional tier. (This section originally said registration was deliberately deferred to a separate centralized process — that was true when this proposal was drafted but is stale; corrected here so a future reader doesn't re-verify or re-run wiring that already happened.)

### Verification performed

The `dart run saropa_lints scan` CLI could not be used for the original verification pass since, at that point in implementation, the rule was not yet registered in `_allRuleFactories` (registration was completed later — see "Registration" above, corrected 2026-09-04). Verification was instead done via the `resolved_rule_harness` test oracle (full type/element resolution, no dependency on global registration):
...
## Follow-up (2026-09-04): all recommendations actioned

- **Registration doc corrected** — see "Registration (corrected 2026-09-04)" above.
- **`_isStored` extended** in `lib/src/rules/code_quality/mutable_tearoff_rules.dart` to also recognize: `ConstructorFieldInitializer` (constructor initializer-list assignment), `ReturnStatement`/`ExpressionFunctionBody` (return / arrow-body), and `ListLiteral`/`SetOrMapLiteral`/`MapLiteralEntry`/`RecordLiteral`/`NamedExpression`-under-`RecordLiteral` (collection and record literal elements). Compound assignment (`??=`) was already covered by the existing `AssignmentExpression` branch — confirmed by test, not a code change.
- **Rule dartdoc updated** to list what's now in scope and to explicitly call out the two remaining out-of-scope cases (chained receivers; a tear-off passed as a named argument to a call that itself retains it beyond one shot) so the gap is documented rather than silent.
- **Fixture extended** (`example/lib/code_quality/mutable_tearoff_fixture.dart`) with 3 new `expect_lint` BAD cases: list-literal element, arrow-bodied return, constructor initializer-list assignment.
- **12 new unit tests added** (`test/rules/code_quality/mutable_tearoff_test.dart`, 23/23 passing): list literal, map literal entry, record literal (positional), return statement, arrow-bodied return, constructor initializer-list, compound (`??=`) assignment, top-level variable receiver, static field receiver (unqualified — a qualified `Class.field.method` reference is a 3-identifier chain and confirmed NOT to fire, consistent with the documented chained-receiver limitation), extension method tear-off, import-prefixed call (confirmed non-firing, no crash), and a named-argument-to-a-one-shot-call negative case (confirmed the `NamedExpression`-under-`RecordLiteral` guard does not over-fire on call arguments).
- Verified via `dart test test/rules/code_quality/mutable_tearoff_test.dart` — 23/23 passing.
