...
A limitation surfaced during verification: the `dart run saropa_lints scan` CLI fires `require_platform_check` zero times — even on the pre-existing known-bad fixture — because scan mode parses with an unresolved AST in which implicit (no-`new`) constructor calls are `MethodInvocation` nodes, never `InstanceCreationExpression`. The rule's behavior was therefore verified through the unit test that drives the scanner logic directly. That scan-engine limitation is tracked separately in `plans/history/2026.06/2026.06.19/infra_scan_cli_misses_instance_creation_rules_unresolved_ast.md` (fixed 2026-06-19 via the `scan --resolve` flag).
