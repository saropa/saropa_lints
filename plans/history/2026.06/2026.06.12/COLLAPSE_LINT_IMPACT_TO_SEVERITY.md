...
**Status:** **FULLY CLOSED — all SEV-01..04 follow-ups landed; archived 2026-06-14.** The collapse itself landed in [Unreleased] on 2026-05-03; the four follow-up items (SEV-01 severity audit, SEV-02 quality-gate example, SEV-03 CLI rename, SEV-04 DX scripts) all closed by 2026-06-12. This document captures the rationale and the migration path.
**Owner:** delivered.
**Created:** 2026-05-03.
**Closed:** 2026-06-14 (SEV-01 Bucket B completed in commits `8abafcd7` + `bb9a379d` on 2026-06-12; parent-plan close-out missed at the time and reconciled here).
...
## Follow-up queue — ALL CLOSED

- [x] **SEV-01 (P1)** Audit `severity:` assignments rule-by-rule against MUST-fix / should-fix / info guidance. **Bucket A DONE 2026-06-10** (46 downgraded ERROR→WARNING, ~32 confirmed-correct ERRORs kept, each read individually with an inline `// SEV-01 (downgraded from ERROR):` why-comment). **Bucket B DONE 2026-06-12** (commits `8abafcd7` + `bb9a379d`): all 46 under-rated WARNING candidates audited for false positives, **10 flipped WARNING→ERROR** (websocket/stream/dispose leaks, `avoid_not_encodable_in_to_json`, `avoid_drift_update_without_where`, iOS permission-description rules, `avoid_flashing_content`, `avoid_path_traversal`) — each gated on near-deterministic detection plus a genuine crash/data-loss/leak; **15 held at WARNING** with an inline `// SEV-01 (kept WARNING):` comment naming the residual false positive (mostly cleanup-in-helper that single-method AST scanning can't see — tracked under [`INTERPROCEDURAL_CLEANUP_TRACKING.md`](INTERPROCEDURAL_CLEANUP_TRACKING.md)); 21 permanent KEEP_WARNING. Per-rule verdicts recorded in the archived audit: [`SEV01_SEVERITY_AUDIT.md`](SEV01_SEVERITY_AUDIT.md). The same pass recovered a regression (a dropped closing brace that had silently broken 11 rule classes). Verified: affected rule + integrity tests pass, `dart analyze --fatal-infos` clean.
...
- Robustness-fallback (conditional crash only): `require_error_boundary`, `require_unknown_route_handler`, `require_deep_link_fallback`, `require_go_router_fallback_route`, `avoid_navigator_context_issue`.
- Deployment-config heuristics: `require_android_manifest_entries`, `require_notification_icon_kept`, `require_ios_info_plist_entries`, `require_macos_entitlements`, `require_purchase_restoration`, `avoid_audio_in_background_without_config`, `avoid_geolocator_background_without_config`, `require_hive_web_subdirectory`, `require_firestore_index`, `require_firebase_composite_index`, `require_cors_handling`, `avoid_sqflite_type_mismatch`.
- Accessibility / test-quality / debug-hygiene: `avoid_hidden_interactive`, `require_test_widget_pump`, `avoid_print_in_release`.

**~32 confirmed-correct ERRORs kept** (read and judged genuine MUST-fix): compile errors (`compile_time_syntax_rules.dart` ×5, `uri_does_not_exist`, `duplicate_field_name`, `invalid_field_name`, `invalid_extension_argument_count`, the two SDK-migration `avoid_extending_*`), deterministic crashes (`avoid_setstate_in_build`, `avoid_recursive_widget_calls`, `require_provider_scope`, `avoid_circular_redirects`, `require_mounted_check`, `avoid_optional_field_crash`, `avoid_ref_in_dispose`, `prefer_platform_io_conditional`, `prefer_url_launcher_uri_over_string`, `require_initial_state`), data corruption/loss (`avoid_isar_enum_field`, `avoid_isar_schema_breaking_changes`, `require_isar_nullable_field`, `avoid_cached_isar_stream`), security/exploit (`require_https_only`, `require_route_guards`), disposal-leak family (`require_bloc_close`, `require_scroll_controller_dispose`), and the intentional `prefer_fail_test_case` hook.

**Verification:** `dart test` over the affected rule suites (packages, ui, platforms, security, core, flow, testing + the false-positive integrity test) → **2286 passing**; `dart analyze --fatal-infos` on a sample of changed files → clean. No test asserts severity for any downgraded rule (severity is independent of detection + message). The 23 changed rule files were committed by a concurrent session; the work is intact in HEAD and re-verified here.
...
3. **Per-rule sign-off.** Present the classified list (consequence × precision) to the maintainer; upgrade in small batches with the FP analysis attached, never a bulk flip.
4. **Reconcile the impact getter too** — Bucket B rules currently have severity=WARNING but explicit impact=error; whichever way each is resolved, leave severity and impact agreeing.

**No bug archive** — task did not close a `bugs/*.md` file.

## Finish Report (2026-06-10, Bucket A completion)

Continuation of the above: completed the SEV-01 Bucket A audit (46 rules downgraded, ~32 kept, all read individually; 2286 tests pass; analyze clean) and wrote the Bucket B false-positive-first plan. Bucket B not started — it is the only remaining SEV-01 scope and needs per-rule sign-off. Plan stays active (Bucket B open; canonical tracker referenced from CHANGELOG).
