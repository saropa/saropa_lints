...
2. **Quick fix added** — `WrapTextInExpandedFix` wraps a bare `Text(...)` in `Expanded(child: ...)` via the IDE lightbulb menu. Registered on both `RequireTextOverflowHandlingRule` and `RequireTextOverflowInRowRule`. This gives AI agents and IDE users a one-click fix that preserves text visibility.
...
**Files changed:**
- `lib/src/rules/widget/widget_patterns_require_rules.dart` — correctionMessage rewritten, fixGenerators added
- `lib/src/rules/widget/forms_rules.dart` — correctionMessage rewritten, DartDoc GOOD example updated, fixGenerators added
- `lib/src/fixes/widget/wrap_text_in_expanded_fix.dart` — NEW: quick fix implementation
