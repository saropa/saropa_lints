...
named in each theme already has a proposal filed in `bugs/proposal_*.md` (298 total, see
...
|---|---|---|---|
| 6 | JSON-codegen annotation-contract enforcement | 2 | Narrow, mechanical: annotation present → member must exist. `proposal_json_serializable_enforcement_rules.md`. |
...
|---|---|---|---|---|
| 2 | Generic, user-configurable architecture/import-boundary engine | 30+ | **The single most-repeated pattern in the entire audit** — 6 independent packages built the same kind of engine. Closing this as one configurable engine (not 6 separate proposals) would out-compete the whole category at once. | A project-configurable (YAML-driven) rule engine: layer graph, naming-per-role, base-type requirements. Proposals already scoped: `proposal_architecture_lints_enforcement_rules.md`, `proposal_clean_architecture_enforcement_rules.md`, `proposal_infra_configurable_import_boundary_dsl.md`, `proposal_infra_configurable_widget_ban_mechanism.md`, `proposal_infra_configurable_class_naming_rules.md`. |
...
|---|---|---|---|
| 13 | Niche third-party package APIs (context_plus, logd, all_observer, mad_lint) | ~57 | Each targets one company's/package's internal API surface with no broader applicability. Proposals exist (`proposal_context_plus_ref_validation_rules.md`, `proposal_logd_linters_rules.md`, `proposal_mad_lint_mapped_fields_rules.md`) but only worth building if saropa specifically wants to support that dependency. |
