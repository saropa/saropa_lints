...
Future<void> main(List<String> args) async {
  ...
  // ── Fix trailing comments on ignore directives ──
  // Always runs (not v4-specific) because trailing text silently breaks
  // suppression regardless of plugin version.
  _logTerminal('');
  _logTerminal(
    '${_Colors.bold}Scanning for broken ignore comments...${_Colors.reset}',
  );
  final Map<String, int> trailingResults = _fixTrailingIgnoreComments(
    cliArgs.dryRun,
  );
  if (trailingResults.isEmpty) {
    _logTerminal(
      '${_Colors.dim}  No trailing-comment issues found${_Colors.reset}',
    );
  } else {
    final int total = trailingResults.values.fold(0, (s, c) => s + c);
    final String verb = cliArgs.dryRun ? 'Found' : 'Fixed';
    _logTerminal(
      '${_Colors.green}$verb $total trailing ignore comment'
      '${total == 1 ? '' : 's'} in '
      '${trailingResults.length} file'
      '${trailingResults.length == 1 ? '' : 's'}${_Colors.reset}',
    );
...
/// Scans `lib/`, `test/`, `bin/` for ignore directives with trailing
/// comments that silently break suppression.
///
/// For example, `// ignore: rule // reason` causes the framework to store
/// `rule // reason` as the code, which never matches the actual diagnostic.
///
/// Fixes by moving the trailing text to the line above the directive.
/// Returns a map of file path to number of fixes applied.
///
/// Does not scan `example*/` directories — those contain intentional
/// violations for test fixtures and are excluded from analysis.
Map<String, int> _fixTrailingIgnoreComments(bool dryRun) {
  final Map<String, int> results = <String, int>{};
  ...
  for (final String dirName in const ['lib', 'test', 'bin']) {
    ...
    for (final FileSystemEntity entity in dir.listSync(recursive: true)) {
      if (entity is! File || !entity.path.endsWith('.dart')) continue;
      final int count = _fixTrailingIgnoreCommentsInFile(entity, dryRun);
      if (count > 0) {
        results[entity.path] = count;
      }
    }
...
/// Matches a full line containing a trailing-comment ignore directive.
///
/// Group 1: leading whitespace (for indentation preservation).
/// Group 2: optional code prefix before the ignore comment (e.g. `doSomething(); `).
/// Group 3: the full ignore directive including trailing text.
///
/// Handles both standalone comments and inline ignores on code lines:
/// - `  // ignore: rule // reason`       → groups: (`  `, ``, `// ignore: ...`)
/// - `  doSomething(); // ignore: rule // reason` → groups: (`  `, `doSomething(); `, `// ignore: ...`)
final RegExp _trailingIgnoreLinePattern = RegExp(
  r'^([ \t]*)'
  r'((?:(?!//).)*?)'
...
/// Matches the ` - ` separator in trailing ignore explanations.
final RegExp _dashSeparatorPattern = RegExp(r'\s+-\s+');

/// Fixes trailing comments on ignore directives in a single file.
///
/// Handles both standalone and inline ignore comments:
/// ```
/// // ignore: rule // reason       →  // reason\n// ignore: rule
/// doThing(); // ignore: rule // r →  // r\ndoThing(); // ignore: rule
/// ```
/// Returns count of fixes applied.
int _fixTrailingIgnoreCommentsInFile(File file, bool dryRun) {
  final String content = file.readAsStringSync();
  int count = 0;

  final String newContent = content.replaceAllMapped(
    _trailingIgnoreLinePattern,
    (Match match) {
      final String indent = match.group(1)!;
      final String codePrefix = match.group(2)!;
      final String fullDirective = match.group(3)!;

      // Should always succeed since the regex pre-filters, but guard anyway.
      final parts = _splitIgnoreParts(fullDirective);
      if (parts == null) return match.group(0)!;

      count++;
      return '$indent${parts.trailing}\n'
          '$indent$codePrefix${parts.directive}';
    },
  );
...
/// Splits a trailing-comment ignore directive into its parts.
///
/// Handles both patterns:
/// - `// ignore: rule // reason` → (directive: `// ignore: rule`, trailing: `// reason`)
/// - `// ignore: rule - reason` → (directive: `// ignore: rule`, trailing: `// reason`)
///
/// Returns `null` if no trailing comment or separator is found.
///
/// NOTE: Intentionally duplicated from [AvoidIgnoreTrailingCommentRule.splitParts]
/// in `lib/src/rules/code_quality_rules.dart`. bin/init.dart cannot import that
/// file without pulling in analyzer infrastructure. Update both if this changes.
({String directive, String trailing})? _splitIgnoreParts(String lexeme) {
  final int colonIndex = lexeme.indexOf(':');
  if (colonIndex < 0) return null;

  final String afterColon = lexeme.substring(colonIndex + 1);

  // Check for trailing // comment first (higher priority)
  final int trailingSlashIndex = afterColon.indexOf('//');
  if (trailingSlashIndex >= 0) {
    final String directive = lexeme
        .substring(0, colonIndex + 1 + trailingSlashIndex)
        .trimRight();
    final String trailing = afterColon.substring(trailingSlashIndex).trim();
    return (directive: directive, trailing: trailing);
  }

  // Check for trailing - separator (space-hyphen-space)
  final RegExpMatch? dashMatch = _dashSeparatorPattern.firstMatch(afterColon);
  if (dashMatch != null) {
    final String directive = lexeme
        .substring(0, colonIndex + 1 + dashMatch.start)
        .trimRight();
    final String text = afterColon.substring(dashMatch.end).trim();
