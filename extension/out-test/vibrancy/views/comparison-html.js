...
exports.buildComparisonHtml = buildComparisonHtml;
...
const ROWS = [
    ...
    },
    {
        ...
        extract: p => (0, html_utils_1.escapeHtml)(p.latestVersion || '—'),
    },
    {
        label: 'Published',
    ...
    },
    {
        ...
        extract: p => p.publisher
            ? `<span class="verified">${(0, html_utils_1.escapeHtml)(p.publisher)} ✓</span>`
            : '— (unverified)',
    },
    {
        label: 'Pub Points',
    ...
    },
    {
        ...
        extract: p => p.archiveSizeBytes !== null ? (0, bloat_calculator_1.formatSizeMB)(p.archiveSizeBytes) : '—',
    },
    {
        label: 'Bloat Rating',
    ...
    {
        label: 'License',
        extract: p => (0, html_utils_1.escapeHtml)(p.license ?? '—'),
    },
    {
        label: 'Platforms',
...
function buildHeaderRow(packages) {
    const cells = packages.map(pkg => {
        ...
        const addBtn = !pkg.inProject
            ? `<br><button class="add-btn" data-package="${(0, html_utils_1.escapeHtml)(pkg.name)}" data-version="${(0, html_utils_1.escapeHtml)(pkg.latestVersion)}">Add to Project</button>`
            : '';
        return `<th class="pkg-header"><a href="${url}">${(0, html_utils_1.escapeHtml)(pkg.name)}</a>${inProjectBadge}${addBtn}</th>`;
    }).join('');
    return `<tr><th class="dimension"></th>${cells}</tr>`;
}
function buildDataRow(row, packages, winners) {
    ...
    const cells = packages.map(pkg => {
        ...
        const isWinner = row.dimension
            ? (0, comparison_ranker_1.isWinnerForDimension)(pkg.name, row.dimension, winners)
            : false;
...
function buildComparisonTable(packages, winners) {
...
function buildComparisonHtml(ranked) {
