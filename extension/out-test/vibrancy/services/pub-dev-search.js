...
exports.searchAlternatives = searchAlternatives;
exports.buildAlternatives = buildAlternatives;
 ...
 */
async function searchAlternatives(topics, exclude, cache, logger) {
    if (topics.length === 0) {
        return [];
    }
...
async function searchByTopics(topics, logger) {
    ...
    try {
        ...
        const t0 = Date.now();
        const resp = await (0, fetch_retry_1.fetchWithRetry)(url, undefined, logger);
        ...
        const scores = await Promise.all(topPackages.map(pkg => fetchPackageScore(pkg.package, logger)));
...
async function fetchPackageScore(name, logger) {
...
async function filterAndRank(results, exclude, cache, logger) {
    const excludeSet = new Set(exclude.map(n => n.toLowerCase()));
    const filtered = results.filter(r => !excludeSet.has(r.name.toLowerCase()));
    const suggestions = [];
...
async function getVibrancyScore(name, cache, _logger) {
 ...
 */
function buildAlternatives(curatedReplacement, discoveryAlternatives) {
    const alternatives = [];
...
//# sourceMappingURL=pub-dev-search.js.map