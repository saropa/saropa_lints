...
exports.DetailLogger = exports.DETAIL_CHANNEL_NAME = void 0;
...
exports.DETAIL_CHANNEL_NAME = 'Vibrancy Details';
...
class DetailLogger {
    ...
    _logPackageContent(result, timestamp) {
        const displayScore = Math.round(result.score / 10);
        const catLabel = (0, status_classifier_1.categoryLabel)(result.category);
    ...
    _logVersionInfo(result, timestamp) {
        ...
        if (result.archiveSizeBytes !== null) {
            this._logWithTimestamp(timestamp, `Size: ${(0, bloat_calculator_1.formatSizeMB)(result.archiveSizeBytes)}`);
        }
    }
    _logUpdateInfo(result, timestamp) {
    ...
    _logSuggestions(result, timestamp) {
        const suggestions = [];
        if (result.knownIssue?.replacement) {
            const displayReplacement = (0, known_issues_1.getReplacementDisplayText)(result.knownIssue.replacement, result.package.version, result.knownIssue.replacementObsoleteFromVersion);
    ...
    _logCommunityInfo(result, timestamp) {
        const lines = [];
        if (result.github) {
            ...
            if (gh.daysSinceLastCommit !== undefined) {
                lines.push(`Last commit: ${(0, time_formatter_1.formatRelativeTime)(gh.daysSinceLastCommit)}`);
            }
            if (gh.isArchived) {
    ...
    _logWithTimestamp(timestamp, text) {
        this._channel.appendLine(`[${timestamp}] ${text}`);
    }
    _logIndented(timestamp, text) {
...
exports.DetailLogger = DetailLogger;
...
function formatFlaggedIssue(issue) {
...
//# sourceMappingURL=detail-logger.js.map