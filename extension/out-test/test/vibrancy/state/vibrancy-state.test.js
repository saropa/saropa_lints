...
const vibrancy_state_1 = require("../../../vibrancy/state/vibrancy-state");
...
describe('VibrancyStateManager', () => {
    ...
    beforeEach(() => {
        ...
        manager = new vibrancy_state_1.VibrancyStateManager();
    });
    afterEach(() => {
        manager.dispose();
        (0, vscode_mock_1.clearTestConfig)();
    ...
    describe('initial state', () => {
        ...
        it('should respect enableCodeLens setting', () => {
            manager.dispose();
            (0, vscode_mock_1.setTestConfig)('saropaLints.packageVibrancy', 'enableCodeLens', false);
            manager = new vibrancy_state_1.VibrancyStateManager();
    ...
    describe('STATE_KEYS', () => {
        it('should have correct key prefixes', () => {
            assert.ok(vibrancy_state_1.STATE_KEYS.hasResults.startsWith('saropaLints.packageVibrancy.'));
            assert.ok(vibrancy_state_1.STATE_KEYS.isScanning.startsWith('saropaLints.packageVibrancy.'));
            assert.ok(vibrancy_state_1.STATE_KEYS.codeLensEnabled.startsWith('saropaLints.packageVibrancy.'));
        });
    });
    ...
    describe('syncAll', () => {
        it('should sync all states to context', () => {
            contextValues = {};
            manager.syncAll();
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.hasResults], false);
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.isScanning], false);
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.packageCount], 0);
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.updatableCount], 0);
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.problemCount], 0);
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.codeLensEnabled], true);
            assert.strictEqual(contextValues[vibrancy_state_1.STATE_KEYS.selectedPackage], null);
        });
    });
    describe('dispose', () => {
...
//# sourceMappingURL=vibrancy-state.test.js.map