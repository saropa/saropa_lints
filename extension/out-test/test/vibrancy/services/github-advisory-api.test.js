...
const github_advisory_api_1 = require("../../../vibrancy/services/github-advisory-api");
describe('github-advisory-api', () => {
    ...
    describe('queryGitHubAdvisories', () => {
        it('should return empty array for empty package list', async () => {
            const results = await (0, github_advisory_api_1.queryGitHubAdvisories)([]);
            assert.deepStrictEqual(results, []);
            assert.strictEqual(fetchStub.callCount, 0);
        });
        it('should query GitHub API for each package', async () => {
            fetchStub.resolves(new Response(JSON.stringify([]), { status: 200 }));
            await (0, github_advisory_api_1.queryGitHubAdvisories)([
        ...
        it('should parse advisory response correctly', async () => {
            ...
            fetchStub.resolves(new Response(JSON.stringify([mockAdvisory]), { status: 200 }));
            const results = await (0, github_advisory_api_1.queryGitHubAdvisories)([
        ...
        it('should include authorization header when token provided', async () => {
            fetchStub.resolves(new Response(JSON.stringify([]), { status: 200 }));
            await (0, github_advisory_api_1.queryGitHubAdvisories)([{ name: 'pkg', version: '1.0.0' }], 'test-token');
        ...
        it('should use cache for repeated queries', async () => {
            ...
            await (0, github_advisory_api_1.queryGitHubAdvisories)([{ name: 'cached_pkg', version: '1.0.0' }], undefined, mockCache);
        ...
        it('should process packages in parallel batches', async () => {
            ...
            await (0, github_advisory_api_1.queryGitHubAdvisories)(packages);
            assert.strictEqual(fetchStub.callCount, 10);
        });
    });
    describe('mergeVulnerabilities', () => {
        it('should return empty array for empty sources', () => {
            const result = (0, github_advisory_api_1.mergeVulnerabilities)([], []);
        ...
        it('should combine vulnerabilities from multiple sources', () => {
            ...
            const result = (0, github_advisory_api_1.mergeVulnerabilities)(osv, ghsa);
        ...
        it('should deduplicate by ID', () => {
            ...
            const result = (0, github_advisory_api_1.mergeVulnerabilities)(source1, source2);
            assert.strictEqual(result.length, 1);
            assert.strictEqual(result[0].id, 'GHSA-xxxx-yyyy-zzzz');
        });
    });
});
//# sourceMappingURL=github-advisory-api.test.js.map