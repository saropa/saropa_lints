...
const known_issues_json_1 = __importDefault(require("../../../vibrancy/data/known_issues.json"));
const known_issues_1 = require("../../../vibrancy/scoring/known-issues");
/** Flatten grouped map into a flat array of [name, issue] pairs. */
function flatEntries() {
    const result = [];
    for (const [name, issues] of (0, known_issues_1.allKnownIssues)()) {
...
describe('known-issues', () => {
    it('should have unique name+version tuples', () => {
        const issues = known_issues_json_1.default.issues;
        const keys = issues.map((e) => `${e.name}|${e.appliesToMinVersion ?? ''}|${e.appliesToMaxVersion ?? ''}`);
        const dupes = keys.filter((k, i) => keys.indexOf(k) !== i);
        assert.deepStrictEqual(dupes, [], `duplicate name+version tuples in known_issues.json: ${dupes.join(', ')}`);
    });
    it('should find a known bad package', () => {
        const issue = (0, known_issues_1.findKnownIssue)('flutter_datetime_picker');
        assert.ok(issue);
    ...
    it('should return null for unknown packages', () => {
        assert.strictEqual((0, known_issues_1.findKnownIssue)('totally_made_up_pkg'), null);
    });
    it('should load all known issues', () => {
        const all = (0, known_issues_1.allKnownIssues)();
    ...
    it('should find an active status package', () => {
        const issue = (0, known_issues_1.findKnownIssue)('http');
        assert.ok(issue);
    ...
    it('should parse overrideReason when present', () => {
        const issue = (0, known_issues_1.findKnownIssue)('path_provider_foundation');
        assert.ok(issue);
    ...
    it('should have migrationNotes when replacement is present', () => {
        for (const [name, issue] of flatEntries()) {
            if (issue.replacement) {
                assert.ok(issue.migrationNotes, `${name}: has replacement but missing migrationNotes`);
            }
        }
    });
    describe('version-scoped lookup', () => {
        it('should match scoped entry when version falls in range', () => {
            const issue = (0, known_issues_1.findKnownIssue)('flutter_local_notifications', '8.5.0');
        ...
        it('should match different scoped entry for different version', () => {
            const issue = (0, known_issues_1.findKnownIssue)('flutter_local_notifications', '9.2.0');
        ...
        it('should return null when version outside all ranges and no unscoped entry', () => {
            ...
            const issue = (0, known_issues_1.findKnownIssue)('flutter_local_notifications', '10.0.0');
        ...
        it('should prefer scoped entry over unscoped for matching version', () => {
            ...
            const v2 = (0, known_issues_1.findKnownIssue)('timeago', '2.5.0');
            assert.ok(v2, 'expected scoped v2 match');
            assert.strictEqual(v2.appliesToMinVersion, '2.0.0');
            const v4 = (0, known_issues_1.findKnownIssue)('timeago', '4.0.0');
        ...
        it('should treat min as inclusive', () => {
            ...
            const issue = (0, known_issues_1.findKnownIssue)('flutter_local_notifications', '8.0.0');
        ...
        it('should treat max as exclusive', () => {
            ...
            const issue = (0, known_issues_1.findKnownIssue)('flutter_local_notifications', '9.0.0');
            assert.ok(issue);
            assert.strictEqual(issue.appliesToMinVersion, '9.0.0');
        });
    });
    describe('isReplacementPackageName', () => {
        it('should return true for pub package names', () => {
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('dio'), true);
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('path_provider'), true);
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('flutter_secure_storage'), true);
        });
        it('should return false for upgrade instructions and freeform text', () => {
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('Update to v9+'), false);
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('Update to latest version'), false);
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('Use Native Channels'), false);
            assert.strictEqual((0, known_issues_1.isReplacementPackageName)('Native `showDialog`'), false);
        });
    });
    describe('getReplacementDisplayText', () => {
        it('should return replacement when no obsolete-from-version or when version below', () => {
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('dio', '1.0.0'), 'dio');
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to latest version', '1.0.0'), 'Update to latest version');
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '8.0.0', '9.0.0'), 'Update to v9+');
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '5.0.0', '9.0.0'), 'Update to v9+');
        });
        it('should return undefined when replacementObsoleteFromVersion set and current >= it', () => {
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '10.0.0', '9.0.0'), undefined);
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '9.0.0', '9.0.0'), undefined);
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '9.1.0', '9.0.0'), undefined);
        });
        it('should support single-segment threshold (e.g. "9")', () => {
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '10', '9'), undefined);
            assert.strictEqual((0, known_issues_1.getReplacementDisplayText)('Update to v9+', '8', '9'), 'Update to v9+');
        });
    });
});
//# sourceMappingURL=known-issues.test.js.map