...
function extractRowData(html: string): unknown[] {
  // Case-insensitive + attribute-tolerant closing tag (`[^>]*`) to satisfy
  // CodeQL js/bad-tag-filter. This is test extraction, not security sanitization.
  const m = html.match(/<script id="pvRowData"[^>]*>([\s\S]*?)<\/script[^>]*>/i);
