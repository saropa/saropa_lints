...
import {
    ProblemRegistry, generateProblemId,
    UnhealthyPackageProblem, BlockedUpgradeProblem,
    UnusedProblem,
...
describe('generateProblemId', () => {
    it('should generate ID without suffix', () => {
        const id = generateProblemId('my_package', 'unhealthy');
        assert.strictEqual(id, 'my_package:unhealthy');
    });

    it('should generate ID with suffix', () => {
        const id = generateProblemId('my_package', 'vulnerability', 'CVE-2024-1234');
