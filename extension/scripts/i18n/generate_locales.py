...
def write_coverage_json(stats_by_locale: dict[str, LocaleStats], root: Path) -> None:
    """Write per-locale coverage to a JSON the extension imports at build time.

    The UI language picker reads this file to badge incomplete locales (e.g.
    "4% translated") so a user can never silently select a near-English UI. The
    numbers come from the SAME LocaleStats the audit/translate run computes, so
    the badge can never drift from measured reality.

    A partial run (``--locales bn,de``) only has stats for the locales it touched.
    Merge into any existing file rather than overwriting so those runs update just
    their own entries and leave the rest intact. English is always 100% — it is
    the source, never a translation target, so it never appears in stats_by_locale
    and is injected here.
    """
    path = root / "src" / "i18n" / "locale_coverage.json"
    existing: dict[str, object] = {}
    if path.is_file():
        try:
            existing = read_json(path)
        except (ValueError, OSError):
            # A corrupt/partial prior file must not abort a translation run;
            # rebuild from scratch instead of crashing the caller.
            existing = {}
    locales: dict[str, dict[str, int]] = dict(existing.get("locales", {}))  # type: ignore[arg-type]
    locales["en"] = {"coveragePct": 100, "missing": 0}
    for locale, s in stats_by_locale.items():
        needs = max(0, s.total - s.skipped)
        pct = 100 if needs == 0 else round(100 * s.translated / needs)
        locales[locale] = {"coveragePct": pct, "missing": s.missing}
    payload = {
        "generated": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "note": (
            "GENERATED by extension/scripts/i18n/generate_locales.py (audit or "
            "translate run). Do not hand-edit; the UI language picker badges any "
            "locale below 100% from coveragePct. English is the source and is "
            "always 100%."
        ),
...
def _run_audit(
    ...
    *,
    ...
    # Refresh the picker's coverage source from this audit so the language-picker
    # badges always reflect the latest measured reality, even on an audit-only run.
    write_coverage_json(stats_by_locale, root)
...
def main() -> int:
    ...
    if stats_by_locale:
        ...
        # Refresh the picker's coverage source so newly-completed locales drop
        # their badge and regressed ones gain one, straight from this run's stats.
        write_coverage_json(stats_by_locale, root)
        total_missing = sum(s.missing for s in stats_by_locale.values())
        print()
