...
- **H4 (Score in Code Lens):** `codeLensProvider.ts` — Code Lens now shows critical count per file: "Saropa: 3 issues (2 critical) — Show in Saropa". Critical annotation only appears when file has critical-impact violations. Title shortened from "Saropa Lints:" to "Saropa:" to fit the annotation.
...
- **D6 (File Risk heatmap):** `fileRiskTree.ts` — files ranked by weighted violation density (same impact weights as `healthScore.ts`: critical=8, high=3, medium=1, low=0.25, opinionated=0.05). Top summary item: "Top N files have X% of critical issues". Each file shows flame icon (critical), warning icon (high), or info icon. Click filters Issues view to that file via `focusIssuesForFile`. Capped at 30 files. New view `saropaLints.fileRisk` registered with welcome states and refresh button.
- **D10 (Group-by in Issues):** `issuesTree.ts` — 5 grouping modes: Severity (default), File, Impact, Rule, OWASP Category. Toolbar "Group by…" button (list-tree icon) opens a quick pick. Non-severity modes use `buildGroupedRoot()` which flattens the severity→file index, groups violations by selected key, and returns `GroupItem[]` (impact/rule/owasp) or `FileItem[]` (file mode). Group children sub-group into files for easy navigation. Mode-aware icons: impact uses severity icons (error/warning/info/circle-outline), OWASP uses shield, rule uses symbol-method. OWASP mode maps violations to multiple categories via `v.owasp.mobile` + `v.owasp.web`; uncategorized group for violations without OWASP mappings. Command `saropaLints.setGroupBy` registered in `extension.ts`.
