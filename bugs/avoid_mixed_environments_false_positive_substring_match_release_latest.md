...
- `release` is inside `release_notes` (and `prerelease`, `releaseDate`, ...)
- `test` is inside `latest` (and `greatest`, `contest`, `attestation`, ...)
- `dev` is inside `developer`, `device`, `deviation`
- `live` is inside `delivery`, `oliveColor`
- `local` is inside `locale`, `localizedName`
...
3. **A `*Settings` with `developerName` + a `liveDelivery` field** — expect NO lint (`dev` in developer, `live` in delivery)
