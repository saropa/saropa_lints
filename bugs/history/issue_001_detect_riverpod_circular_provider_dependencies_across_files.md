...
**Resolved in:** N/A — In-file detection is implemented (`avoid_circular_provider_deps`). Cross-file detection remains deferred (ROADMAP.md Part 2: Deferred Cross-File Analysis Rules); would require provider-dependency graph infrastructure similar to `ImportGraphCache`. Issue closed.
