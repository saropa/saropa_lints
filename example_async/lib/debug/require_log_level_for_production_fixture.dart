...
// Test fixture for: require_log_level_for_production
...
// BAD: Should trigger require_log_level_for_production
// expect_lint: require_log_level_for_production
void _bad310_processOrder(Order order) {
  log('Processing: ${order.toJson()}'); // Leaks data in production!
}

// GOOD: Should NOT trigger require_log_level_for_production
void _good310_processOrder(Order order) {
  if (kDebugMode) {
    log('Processing: ${order.toJson()}');
  }
}

// FALSE POSITIVE: math.log() is a logarithm, not logging
void _fp310_mathLog() {
  final result = math.log(42);
}

// FALSE POSITIVE: unrelated object with .debug() method
void _fp310_objectDebug() {
  myWidget.debug('layout info');
}

// FALSE POSITIVE: .trace() on a non-logger object
