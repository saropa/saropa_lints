...
// Test fixture for: prefer_webview_sandbox
// BAD: WebViewWidget with controller that is never sandboxed → lint.
// GOOD: WebViewWidget with controller configured via setNavigationDelegate /
//       setAllowFileAccess(false) in same file → no lint.

import 'package:saropa_lints_example/flutter_mocks.dart';

// BAD: Should trigger prefer_webview_sandbox (controller never configured).
// expect_lint: prefer_webview_sandbox
void _badWebViewSandboxUnconfigured() {
  final unconfigured = WebViewController();
  return WebViewWidget(controller: unconfigured);
}

// GOOD: Should NOT trigger (controller has setNavigationDelegate).
void _goodWebViewSandboxWithNavigationDelegate() {
  final controller = WebViewController()
    ..setNavigationDelegate(const NavigationDelegate(onNavigationRequest: null));
  return WebViewWidget(controller: controller);
}

// GOOD: Should NOT trigger (controller has setAllowFileAccess(false)).
void _goodWebViewSandboxWithFileAccessDisabled() {
  final controller = WebViewController();
  final platform = controller.platform;
  if (platform is AndroidWebViewController) {
    (controller.platform as AndroidWebViewController).setAllowFileAccess(false);
  }
  return WebViewWidget(controller: controller);
}

// BAD: Second controller is not configured; only first is. Should trigger once.
void _badTwoControllersOnlyOneConfigured() {
  final configured = WebViewController()
    ..setNavigationDelegate(const NavigationDelegate(onNavigationRequest: null));
  final unconfigured = WebViewController();
  return Column(
    children: [
      WebViewWidget(controller: configured),
      // expect_lint: prefer_webview_sandbox
      WebViewWidget(controller: unconfigured),
    ],
  );
}
