...
See [example/analysis_options_template.yaml](https://github.com/saropa/saropa_lints/blob/main/example/analysis_options_template.yaml) for a complete reference with all 2057+ rules organized by category, tier membership, and examples.
...
The scanner reads the project's `analysis_options.yaml` (generated by `init`) to determine which rules to run. Two steps:
...
Results are written to `reports/<date>/<timestamp>_scan_report.log` with a compact summary printed to terminal.
...
- `comprehensive`/`pedantic`: 1825+ rules → **slowest** (everything)
