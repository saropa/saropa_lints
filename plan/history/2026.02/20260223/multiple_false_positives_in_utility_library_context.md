...
The entire purpose of `isUnixEpochDateTime` is to check whether a `DateTime` instance is **precisely** the Unix epoch (`1970-01-01T00:00:00.000Z`). Replacing `==` with `difference().abs() < threshold` would be semantically wrong -- it would match dates that are _near_ the epoch but not the epoch itself.
