...
### Phase C — WOW Tier 2 (1 sprint) *(done 2026-03-14)*
...
**Implementation record:** W7: `focusedFile` filter in IssuesTreeProvider, context menu "Show only this file" on file nodes, toolbar "Show all files". W5: `runHistory.ts` persists last 20 snapshots in workspace state; Overview shows "Trends" row. W6: celebration messages on violation decrease; "↓ N fewer issues" in Overview. W8: second status bar item showing tier, click to change. Status bar logic consolidated into `updateAllStatusBars()`.
