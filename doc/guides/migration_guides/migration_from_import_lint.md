...
|---|---|---|
| `target`/`from`/`except` glob DSL (user-defined import-boundary rules) | TODO | see [proposal](../../../plans/tier_3_infrastructure/proposal_infra_configurable_import_boundary_dsl.md). saropa_lints has no generic, project-configurable import-boundary DSL; it ships fixed rules for specific layering patterns instead (see below). |
