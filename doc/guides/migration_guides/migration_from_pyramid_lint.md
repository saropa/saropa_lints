...
| `no_duplicate_imports` | PARTIAL | `avoid_duplicate_named_imports` — pyramid catches byte-identical duplicate imports regardless of prefix; unclear whether saropa's rule also fires on a verbatim duplicate with no prefix at all. TODO — see [proposal](../../../bugs/tier_1_quick_wins/proposal_no_duplicate_imports.md) |
| `no_self_comparisons` | HAVE | `avoid_self_compare` |
| `prefer_async_await` | HAVE | `prefer_async_await` |
| `prefer_iterable_any` | TODO | TODO — see [proposal](../../../bugs/tier_1_quick_wins/proposal_prefer_iterable_any.md) (`.where().isNotEmpty` → `.any()`; saropa's similarly-named `prefer_any_or_every` is a false cognate — it targets explicit-null named arguments, not iterable-chain rewrites) |
...
| `prefer_border_radius_all` | PARTIAL | `prefer_borderradius_circular` — same subject, contradictory prescribed style (saropa recommends `.circular()` over `.all()`, pyramid recommends the opposite). TODO — see [proposal](../../../bugs/tier_3_infrastructure/proposal_prefer_border_radius_all.md) |
| `prefer_dedicated_media_query_functions` | PARTIAL | `avoid_deprecated_use_inherited_media_query` — TODO extend, see [proposal](../../../bugs/tier_2_high_value/proposal_extend_avoid_deprecated_use_inherited_media_query_dcm_parity.md) |
