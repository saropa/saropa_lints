...
|---|---|---|
| `class_order_rule` | PARTIAL | `prefer_member_ordering` — saropa's rule is a flat 3-bucket member order (fields, constructors, methods, roughly) with an autofix, not the 10-category MVVM-specific ordering (constructor → callback fields → repository fields → final/const/static → late → other mutable → getter/backing-field/setter triad → getter/setter → public methods → private methods) that `class_order_rule` enforces. |
