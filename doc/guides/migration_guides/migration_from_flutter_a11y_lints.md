...
| `A02` — label contains redundant role words ("button"/"icon") | TODO | TODO — no proposal filed yet. |
...
| `A05` — remove redundant `Semantics(button: true)` wrapper on a primitive button | TODO | TODO — no proposal filed yet. |
| `A06` — interactive control with multiple semantic parts should use `MergeSemantics` | HAVE | `prefer_merge_semantics` |
| `A07` — `Semantics()` label replacement must exclude children (prevents double announcement) | TODO | TODO — no proposal filed yet. |
| `A09` — numeric-only label missing units | TODO | TODO — no proposal filed yet. |
| `A13` — composite control should present a single semantic role (2+ focusable descendants) | TODO | TODO — no proposal filed yet. |
...
| `A21` — use `IconButton.tooltip` instead of wrapping with `Tooltip` | TODO | TODO — no proposal filed yet. |
