...
void main() {
  ...
  group('buildRuleMetadataCatalog', () {
    ...
    // Regression test: the VS Code extension's live-diagnostics path
    // (`liveDiagnosticsModel.ts`'s `applyRuleCatalog`) backfills `correction`
    // and `owasp` onto a live Violation from this catalog, since a raw
    // analyzer Diagnostic carries neither. Before this test, the catalog
    // only carried type/lifecycle/security-review fields, so the Findings
    // dashboard's "How to fix" / "OWASP mapping" sections were silently
    // empty on the live workflow. This pins that every rule's snapshot now
