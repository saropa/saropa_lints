...
void main() {
  test('lsp_server.dart source matches liveDiagnosticsModel.ts constant', () {
    ...
    final dartMatch =
        RegExp(r"'source'\s*:\s*'([^']+)'").firstMatch(dartContent);
    ...
    final tsMatch = RegExp(r"LSP_TEST_DIAGNOSTIC_SOURCE\s*=\s*'([^']+)'")
        .firstMatch(tsContent);
    expect(tsMatch, isNotNull,
        reason: 'liveDiagnosticsModel.ts must export '
    ...
    expect(dartSource, equals(tsSource),
        reason: 'Dart diagnostic source must match TS filter constant. '
