...
void main() {
  ...
  test('buildCiWorkflow pins the running version, never a broken ref', () {
    ...
    expect(unknown, contains('references the default branch'));
  });

  // The extension's CI card (extension/src/systemHealth/ciWorkflow.ts) must
  // write the same bytes as --emit-ci. Its test compares against these same
  // fixtures, so a wording change made on one side only fails one of the two.
  group('buildCiWorkflow matches the shared fixtures', () {
    ...
    test('pinned with an explicit tier', () {
      expect(
        buildCiWorkflow(version: '16.3.0', tier: 'recommended'),
        fixture('pinned_tier_recommended.yml'),
      );
    });
