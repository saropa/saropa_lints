...
ConstructorDeclaration _firstConstructor(ClassDeclaration cls) {
  // ClassDeclaration.body is the sealed ClassBody, which splits into
  // BlockClassBody (has `members`) and EmptyClassBody (no body). Test
  // fixtures always declare `{...}` so the body is BlockClassBody. analyzer
  // 12 added `.members` directly onto ClassBody but kept BlockClassBody as a
  // subtype, so this cast stays valid across the supported analyzer 9-12
  // range (pubspec caps at <13 — see the analyzer note there for the meta pin
  // that gates analyzer 13).
