...
void main() {
  ...
  group('AvoidImplicitAnimationDisposeCastRule', () {
    ...
    test('is in professional tier (cumulative includes recommended + essential)', () {
    ...
    test('is not in essential tier alone', () {
      ...
      expect(
        ...
        isFalse,
        reason: 'implicit animation dispose cast is polish/migration, not crash-critical tier-1',
      );
    });
    ...
    test('fixture: BAD case has expect_lint before violation', () {
      ...
      expect(
        ...
        isTrue,
        reason: 'Fixture must document the diagnostic site for custom_lint workflows',
      );
    });

    test('fixture: GOOD dispose-only block has no expect_lint (false positive guard)', () {
      final content = File(
        'example_widgets/lib/animation/${ruleName}_fixture.dart',
      ).readAsStringSync();
    ...
    test('fixture: curve-read GOOD block has no expect_lint (non-dispose cast)', () {
      final content = File(
        'example_widgets/lib/animation/${ruleName}_fixture.dart',
      ).readAsStringSync();
