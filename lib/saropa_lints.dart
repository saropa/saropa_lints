...
List<LintRule> get allSaropaRules => _ruleFactories.values.map((f) => f()).toList();
...
String _resolveVersion() {
  try {
    ...
    final versionMatch =
        RegExp(r'^version:\s*(.+)$', multiLine: true).firstMatch(pubspec.readAsStringSync());
...
late final Map<String, LintRule Function()> _ruleFactories = _buildRuleFactoriesMap();
...
class _SaropaLints extends PluginBase {
  ...
  List<LintRule> getLintRules(CustomLintConfigs configs) {
    ...
    final Set<String> loadedNames = filteredRules.map((LintRule r) => r.code.name).toSet();
    ...
    final String effectiveTier = enableAll ? 'all' : _inferEffectiveTier(enabledRuleNames);
