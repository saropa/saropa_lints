    ...
    context.addFunctionExpression((FunctionExpression node) {
      ...
      if (body is ExpressionFunctionBody) {
        ...
        if (statements.length == 1) {
          final st = statements.first;
          if (st is ReturnStatement) {
            bodyExpr = st.expression;
          }
        }
      }
      ...
      final String paramName = params[0].name?.lexeme ?? '';
  ...
  ReturnStatement? _getSingleReturn(Statement statement) {
    ...
    if (statement is Block) {
      ...
      if (statements.length == 1) {
        ...
        if (first is ReturnStatement) return first;
      }
    }
    return null;
  }
