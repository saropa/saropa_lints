...
///
/// Saropa lint rule implementations grouped under `code_quality/`.
///
...
class AvoidContradictoryExpressionsRule extends SaropaLintRule {
  ...
  String? _getNullCheckedVariable(Expression expr) {
    if (expr is BinaryExpression) {
      ...
      if (expr.leftOperand is NullLiteral && right is SimpleIdentifier) {
        return right.name;
      }
      final left = expr.leftOperand;
      if (expr.rightOperand is NullLiteral && left is SimpleIdentifier) {
        return left.name;
      }
    }
    return null;
  ...
  int _editDistance(String a, String b) {
    ...
    for (int i = 1; i <= a.length; i++) {
      for (int j = 1; j <= b.length; j++) {
        ...
        final int v1 = dp[i - 1][j] + 1;
        final int v2 = dp[i][j - 1] + 1;
        final int v3 = dp[i - 1][j - 1] + cost;
        dp[i][j] = v1 < v2 ? (v1 < v3 ? v1 : v3) : (v2 < v3 ? v2 : v3);
      }
    }
  ...
  void runWithReporter(
    ...
    context.addExpressionStatement((ExpressionStatement node) {
      ...
      if (returnType.isDartAsyncFuture && returnType is InterfaceType) {
        final InterfaceType futureType = returnType;
  ...
  static bool _isDeprecated(Element? element) {
    ...
    try {
      ...
      if (element is ConstructorElement) {
        ...
        final enclosingMeta = enclosing.metadata;
  ...
  void runWithReporter(
    ...
    void checkElement(Element? element, AstNode node) {
      try {
        ...
        developer.log(
          'checkElement element API throw',
          name: 'saropa_lints',
          error: e,
          stackTrace: st,
        );
