/// Barrel export for all lint rules organized by category
library;

// Category rule files
export 'accessibility_rules.dart';
export 'animation_rules.dart';
export 'api_network_rules.dart';
export 'architecture_rules.dart';
export 'async_rules.dart';
export 'bluetooth_hardware_rules.dart';
export 'build_method_rules.dart';
export 'class_constructor_rules.dart';
export 'code_quality_avoid_rules.dart';
export 'code_quality_control_flow_rules.dart';
export 'code_quality_prefer_rules.dart';
export 'code_quality_variables_rules.dart';
export 'collection_rules.dart';
export 'complexity_rules.dart';
export 'config_rules.dart';
export 'connectivity_rules.dart';
export 'context_rules.dart';
export 'control_flow_rules.dart';
export 'crypto_rules.dart';
export 'db_yield_rules.dart';
export 'debug_rules.dart';
export 'dependency_injection_rules.dart';
export 'dialog_snackbar_rules.dart';
export 'disposal_rules.dart';
export 'documentation_rules.dart';
export 'equality_rules.dart';
export 'error_handling_rules.dart';
export 'exception_rules.dart';
export 'file_handling_rules.dart';
export 'formatting_rules.dart';
export 'forms_rules.dart';
export 'freezed_rules.dart';
export 'iap_rules.dart';
export 'image_rules.dart';
export 'internationalization_rules.dart';
export 'json_datetime_rules.dart';
export 'lifecycle_rules.dart';
export 'media_rules.dart';
export 'memory_management_rules.dart';
export 'migration_rules.dart';
export 'money_rules.dart';
export 'naming_style_rules.dart';
export 'navigation_rules.dart';
export 'notification_rules.dart';
export 'numeric_literal_rules.dart';
export 'performance_rules.dart';
export 'permission_rules.dart';
export 'platform_rules.dart';
export 'record_pattern_rules.dart';
export 'resource_management_rules.dart';
export 'return_rules.dart';
export 'scroll_rules.dart';
export 'security_auth_storage_rules.dart';
export 'security_network_input_rules.dart';
export 'state_management_rules.dart';
export 'structure_rules.dart';
export 'stylistic_additional_rules.dart';
export 'stylistic_control_flow_rules.dart';
export 'stylistic_error_testing_rules.dart';
export 'stylistic_null_collection_rules.dart';
export 'stylistic_rules.dart';
export 'stylistic_whitespace_constructor_rules.dart';
export 'stylistic_widget_rules.dart';
export 'test_rules.dart';
export 'testing_best_practices_rules.dart';
export 'theming_rules.dart';
export 'type_rules.dart';
export 'type_safety_rules.dart';
export 'ui_ux_rules.dart';
export 'unnecessary_code_rules.dart';
export 'widget_layout_constraints_rules.dart';
export 'widget_layout_flex_scroll_rules.dart';
export 'widget_lifecycle_rules.dart';
export 'widget_patterns_avoid_prefer_rules.dart';
export 'widget_patterns_require_rules.dart';
export 'widget_patterns_ux_rules.dart';

// Platform-specific rule files
export 'platforms/android_rules.dart';
export 'platforms/ios_capabilities_permissions_rules.dart';
export 'platforms/ios_platform_lifecycle_rules.dart';
export 'platforms/ios_ui_security_rules.dart';
export 'platforms/linux_rules.dart';
export 'platforms/macos_rules.dart';
export 'platforms/web_rules.dart';
export 'platforms/windows_rules.dart';

// Package-specific rule files
export 'packages/bloc_rules.dart';
export 'packages/dio_rules.dart';
export 'packages/drift_rules.dart';
export 'packages/equatable_rules.dart';
export 'packages/firebase_rules.dart';
export 'packages/flame_rules.dart';
export 'packages/flutter_hooks_rules.dart';
export 'packages/geolocator_rules.dart';
export 'packages/get_it_rules.dart';
export 'packages/getx_rules.dart';
export 'packages/graphql_rules.dart';
export 'packages/hive_rules.dart';
export 'packages/isar_rules.dart';
export 'packages/package_specific_rules.dart';
export 'packages/provider_rules.dart';
export 'packages/qr_scanner_rules.dart';
export 'packages/riverpod_rules.dart';
export 'packages/shared_preferences_rules.dart';
export 'packages/sqflite_rules.dart';
export 'packages/supabase_rules.dart';
export 'packages/auto_route_rules.dart';
export 'packages/url_launcher_rules.dart';
export 'packages/rxdart_rules.dart';
export 'packages/workmanager_rules.dart';
