...
class AvoidDiInWidgetsRule extends SaropaLintRule {
  ...
  static final _widgetSuperclassRegex = RegExp(r'\bWidget\b');
  static final _stateSuperclassRegex = RegExp(r'\bState\s*<');
  static final _getItRegex = RegExp(r'\bGetIt\b');
    ...
    while (current != null) {
      if (current is ClassDeclaration) {
        ...
        if (extendsClause != null) {
          ...
          return _widgetSuperclassRegex.hasMatch(superclass) ||
              _stateSuperclassRegex.hasMatch(superclass);
        }
      }
  ...
  bool _isServiceLocatorCall(String targetSource, String methodName) {
    if (_getItRegex.hasMatch(targetSource) && methodName == 'call') {
      return true;
    }
    if (_getItRegex.hasMatch(targetSource) && methodName == 'get') {
      return true;
    }
...
class PreferAbstractionInjectionRule extends SaropaLintRule {
  ...
  static final List<RegExp> _concreteRegexes = [
    for (final String p in _concretePatterns)
      RegExp(r'\b' + RegExp.escape(p) + r'\b'),
  ];
  ...
  bool _isLikelyConcrete(String typeName) {
    for (final RegExp re in _concreteRegexes) {
      if (re.hasMatch(typeName)) return true;
    }
    return false;
  }
}
