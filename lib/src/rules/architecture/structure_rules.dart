...
class PreferStaticMethodRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addMethodDeclaration((MethodDeclaration node) {
      ...
      if (node.thisOrAncestorOfType<ExtensionDeclaration>() != null ||
...
class AvoidClassesWithOnlyStaticMembersRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final List<ClassMember> members = node.bodyMembers;
...
class AvoidSettersWithoutGettersRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addMethodDeclaration((MethodDeclaration node) {
      ...
      for (final ClassMember m in classDecl.bodyMembers) {
        if (m is FieldDeclaration) {
  ...
  void runWithReporter(
    ...
    void checkMembers(List<ClassMember> members) {
      ...
      for (final name in setterIndexes.keys) {
        ...
        final setterNode = setterNodes[name];
        if (setterIdx == null || setterNode == null) continue;
        if (getterIdx != null && setterIdx < getterIdx) {
          reporter.atNode(setterNode);
        }
      }
    }

    context.addClassDeclaration((ClassDeclaration node) {
      checkMembers(node.bodyMembers);
    });
...
class PreferStaticBeforeInstanceRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      final members = node.bodyMembers;
...
class PreferMixinOverAbstractRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      if (node.extendsClause != null) return;
      if (node.bodyMembers.isEmpty) return;
      ...
      final constructors = node.bodyMembers
          .whereType<ConstructorDeclaration>()
          .toList();
      final hasGenerative = constructors.any((c) => c.factoryKeyword == null);
      if (hasGenerative) return;

      final abstractKw = node.abstractKeyword;
      if (abstractKw == null) return;
...
class PreferRecordOverTupleClassRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final fields = node.bodyMembers.whereType<FieldDeclaration>().toList();
      ...
      final methods = node.bodyMembers.whereType<MethodDeclaration>();
    ...
    context.addCompilationUnit((CompilationUnit unit) {
      ...
      for (final cls in classes) {
        ...
        if (subclassCount >= 2) {
          ...
          if (abstractKw != null) reporter.atToken(abstractKw);
        }
      }
    });
  }
...
class PreferConstructorsFirstRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      for (int i = 0; i < node.bodyMembers.length; i++) {
        final ClassMember member = node.bodyMembers[i];
        if (member is ConstructorDeclaration) {
      ...
      final ConstructorDeclaration outOfOrder = node.bodyMembers
          .whereType<ConstructorDeclaration>()
          .firstWhere((c) => node.bodyMembers.indexOf(c) > firstMethodIndex);
...
class PreferFactoryBeforeNamedRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      for (int i = firstNamedIndex + 1; i < node.bodyMembers.length; i++) {
        final ClassMember member = node.bodyMembers[i];
...
class PreferOverridesLastRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final ClassMember outOfOrder = node.bodyMembers[lastOverrideIndex];
      reporter.atNode(outOfOrder);
    });
  }
}
...
class PreferExtensionOverUtilityClassRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final List<MethodDeclaration> staticMethods = node.bodyMembers
          .whereType<MethodDeclaration>()
          .where((m) => m.isStatic && !m.isGetter && !m.isSetter)
          .toList();
      if (staticMethods.length < 2) return;

      final bool hasInstanceMember = node.bodyMembers.any((m) {
...
class PreferExtensionTypeForWrapperRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final List<FieldDeclaration> fields = node.bodyMembers
          .whereType<FieldDeclaration>()
          .where((f) => !f.isStatic)
          .toList();
      if (fields.length != 1) return;
      final FieldDeclaration single = fields[0];
      if (!single.fields.isFinal || single.fields.isLate) return;
      if (single.fields.variables.length != 1) return;

      final List<ConstructorDeclaration> constructors = node.bodyMembers
          .whereType<ConstructorDeclaration>()
...
class UnnecessaryLibraryNameRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addLibraryDirective((LibraryDirective node) {
      // LibraryDirective.name is LibraryIdentifier? on analyzer 9-11 and
      // DottedName? on analyzer 12. Both are AstNodes exposing toSource(), so
      // inferring the type keeps this compiling across the supported 9-12
      // range without a version-specific annotation.
