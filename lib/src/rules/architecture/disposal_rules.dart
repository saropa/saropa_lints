...
/// Returns true if [body] contains an assignment of the form
...
bool _initStateAssignsFieldOfType(
  ...
  while (true) {
    ...
    final after = idx + fieldName.length;
    if (after >= body.length) {
      idx = after;
      continue;
    }
    // Word boundary: char before not alphanumeric/_
    if (idx > 0) {
      final code = body.codeUnitAt(idx - 1);
    ...
    if (i + typeName.length > body.length) {
      idx = after;
      continue;
    }
    if (body.substring(i, i + typeName.length) != typeName) {
      idx = after;
      continue;
    }
...
class RequireTextEditingControllerDisposeRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final List<String> fieldNames = _findOwnedFieldsOfType(
        node,
...
class RequireLifecycleObserverRule extends SaropaLintRule {
  RequireLifecycleObserverRule() : super(code: _code);
...
class RequireIntervalTimerCancelRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addMethodInvocation((MethodInvocation node) {
      ...
      FunctionBody? disposeMethodBody;
      for (final ClassMember member in enclosingClass.body.members) {
        if (member is MethodDeclaration && member.name.lexeme == 'dispose') {
          disposeMethodBody = member.body;
          break;
        }
...
class RequireDisposeImplementationRule extends SaropaLintRule {
  RequireDisposeImplementationRule() : super(code: _code);
...
class PreferDisposeBeforeNewInstanceRule extends SaropaLintRule {
  PreferDisposeBeforeNewInstanceRule() : super(code: _code);
  ...
  void runWithReporter(
    ...
    context.addAssignmentExpression((AssignmentExpression node) {
      ...
      if (enclosingMethod.name.lexeme == enclosingClass.namePart.typeName.lexeme) return;
      ...
      final bool hasDisposeBeforeAssignment = _hasDisposeCallBefore(
        enclosingBlock,
        node,
        fieldName,
      );
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      for (final member in node.body.members) {
        if (member is FieldDeclaration) {
          ...
          if (typeName != null) {
            for (var i = 0; i < _disposableTypesList.length; i++) {
              final disposable = _disposableTypesList[i];
              if (_disposableTypeRegexes[i].hasMatch(typeName)) {
