    ...
    for (final ClassMember member in node.members) {
      if (member is FieldDeclaration) {
        ...
        if (match != null) {
          ...
          if (keyType == null || !_isLikelyEnumType(keyType)) {
            return false; // Found a non-enum keyed map or no capture
          }
          hasMapField = true;
        }
...
class RequireCacheKeyUniquenessRule extends SaropaLintRule {
  ...
  static final RegExp _mapKeyTypePattern = RegExp(r'Map<(\w+),');
    ...
    context.addFieldDeclaration((FieldDeclaration node) {
      for (final VariableDeclaration variable in node.fields.variables) {
        ...
        if (typeSource.startsWith('Map<')) {
          final Match? match = _mapKeyTypePattern.firstMatch(typeSource);
          final String? keyType = match?.group(1);
          if (keyType != null && !_safeKeyTypes.contains(keyType)) {
...
class AvoidClosureCaptureLeaksRule extends SaropaLintRule {
  ...
  static bool _hasMountedGuard(
    ...
    for (final Statement stmt in body.block.statements) {
      ...
      if (_mountedPattern.hasMatch(source)) return true;
    }
    return false;
  }
}
