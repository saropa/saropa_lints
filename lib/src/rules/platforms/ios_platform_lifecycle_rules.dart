    ...
    context.addMethodInvocation((MethodInvocation node) {
      ...
      if (methodName == 'periodic') {
        ...
        if (target != null && target.toSource() == 'Timer') {
          // A Timer.periodic that only runs while a widget is on screen, and
          // is torn down alongside it (State.dispose() cancels the field),
          // cannot drain battery in the background -- it stops existing
          // before/while the app backgrounds. Skip those before even
