...
class PreferTryParseForDynamicDataRule extends SaropaLintRule {
  ...
  static const LintCode _code = LintCode(
    ...
    problemMessage:
        '[prefer_try_parse_for_dynamic_data] Calling parse() (such as int.parse, double.parse, DateTime.parse, etc.) on dynamic, user-provided, or external data is dangerous because it throws a FormatException if the input is invalid. This can cause your app to crash or expose users to unhandled errors, especially when processing data from APIs, user forms, or untrusted sources. Using tryParse() instead allows you to safely attempt parsing and handle invalid input gracefully, improving app stability and user experience. This is a common source of runtime exceptions in production apps and is a recommended best practice in Dart and Flutter development.',
    correctionMessage:
        'Replace all uses of parse() on dynamic, user-provided, or external data with tryParse(). Always check for a null result and handle it appropriately, such as by showing a validation error, using a default value, or prompting the user to correct their input. Audit your codebase for parse() calls on untrusted data and refactor to use tryParse() to prevent crashes and improve robustness. Document this practice in your team’s code review checklist to avoid future regressions.',
