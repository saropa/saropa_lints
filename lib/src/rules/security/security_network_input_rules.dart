...
class AvoidLoggingSensitiveDataRule extends SaropaLintRule {
  ...
  @override
  List<int> get cweIds => const <int>[532];
...
class PreferDataMaskingRule extends SaropaLintRule {
  ...
  @override
  List<int> get cweIds => const <int>[359];
...
/// excluded as navigation context, not settings UI.
///
/// Since: v1.7.8 | Updated: v4.13.0 | Rule version: v5
///
...
class AvoidPathTraversalRule extends SaropaLintRule {
  ...
  static final RegExp _pathThrowWord = RegExp(r'\bthrow\b');
  static final RegExp _pathReturnWord = RegExp(r'\breturn\b');
  ...
  bool _hasPathValidation(AstNode node) {
    ...
    while (current != null) {
      ...
      if (_pathTraversalThrowPatterns.any((p) => p.hasMatch(source)) &&
          (_pathThrowWord.hasMatch(source) ||
              _pathReturnWord.hasMatch(source))) {
        return true;
      }
  ...
  bool _returnsUnconditionalTrue(Expression expr) {
    ...
    if (expr is FunctionExpression) {
      ...
      if (body is BlockFunctionBody) {
        ...
        final firstStatement = statements.firstOrNull;
        if (statements.length == 1 && firstStatement is ReturnStatement) {
          final Expression? returnExpr = firstStatement.expression;
...
class RequireHttpsOnlyRule extends SaropaLintRule {
  ...
  static void checkHttpUrls(
    ...
    context.addStringInterpolation((StringInterpolation node) {
      ...
      final InterpolationElement? first = elements.firstOrNull;
      if (first == null || first is! InterpolationString) return;
...
class AvoidStackTraceInProductionRule extends SaropaLintRule {
  ...
  @override
  List<int> get cweIds => const <int>[209];
