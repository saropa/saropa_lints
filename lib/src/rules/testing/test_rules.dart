    ...
    context.addCompilationUnit((CompilationUnit unit) {
      for (final CompilationUnitMember declaration in unit.declarations) {
        ...
        if (declaration is FunctionDeclaration) {
          ...
          name = declaration.nameToken.lexeme;
  ...
  void runWithReporter(
    ...
    context.addBlock((Block block) {
      ...
      for (int i = 0; i < statements.length; i++) {
        ...
        if (interaction == null && expr is MethodInvocation) {
          interaction = expr;
        }
        ...
        if (i + 1 < statements.length) {
          ...
          if (nextStmt is ExpressionStatement) {
            ...
            if (nextExpr is AwaitExpression) {
              final nextInner = nextExpr.expression;
              if (nextInner is MethodInvocation) {
                nextMethod = nextInner;
              }
            }
            if (nextMethod == null && nextExpr is MethodInvocation) {
              nextMethod = nextExpr;
            }
