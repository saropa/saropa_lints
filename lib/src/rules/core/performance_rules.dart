      ...
      for (final ClassMember member in node.bodyMembers) {
        if (member is FieldDeclaration) {
          for (final VariableDeclaration variable in member.fields.variables) {
            ...
            if (typeName != null && globalKeyRegex.hasMatch(typeName)) {
    ...
    if (node.methodName.name == 'map') {
      ...
      for (final Expression arg in node.argumentList.arguments) {
        if (arg is FunctionExpression) {
          ...
          if (widgetCtorRegex.hasMatch(bodySource)) {
            reporter.atNode(node.methodName, code);
          }
        }
      }
      ...
      node.body.visitChildren(
        _OfCallVisitor((MethodInvocation call) {
          ...
          if (target is SimpleIdentifier) {
            ...
            final int count = (ofCallCounts[typeName] ?? 0) + 1;
            ofCallCounts[typeName] = count;
            if (count > 1) {
      ...
      for (final Expression arg in node.argumentList.arguments) {
        if (arg is FunctionExpression) {
          ...
          if (setStateRegex.hasMatch(bodySource) &&
              !mountedRegex.hasMatch(bodySource)) {
            reporter.atNode(node);
          }
        }
      }
...
class RequireDisposePatternRule extends SaropaLintRule {
  ...
  void runWithReporter(
    ...
    context.addClassDeclaration((ClassDeclaration node) {
      ...
      final bool hasConstConstructor = node.bodyMembers.any(
        (ClassMember m) =>
  ...
  void runWithReporter(
    ...
    context.addBinaryExpression((BinaryExpression node) {
      ...
      if (left is SimpleIdentifier) {
        isMoney = _looksFinancial(left.name);
      }

      if (!isMoney) {
        final right = node.rightOperand;
        if (right is SimpleIdentifier) {
          isMoney = _looksFinancial(right.name);
        }
      }
...
class AvoidCacheStampedeRule extends SaropaLintRule {
  ...
  static bool _hasCacheMissThenAwait(Block block) {
    ...
    bool sawAwait = false;
    final containsKeyRegex = RegExp(r'\bcontainsKey\s*\(');
    final notNullRegex = RegExp(r'!=\s*null\b');
    final awaitRegex = RegExp(r'\bawait\s+');
